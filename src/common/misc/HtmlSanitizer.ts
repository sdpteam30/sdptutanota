	...
	private readonly usePlaceholderForInlineImagesRule: Rule = {
		handler: (element, attributeName, attributeValue, config) => {
			if (attributeValue.startsWith("cid:")) {
				// YOUR MobyPhish cid blocking logic
				if (config.blockExternalContent) {
					// Block inline images when blocking external content
					this.externalContent++
					console.log(`🔒 MOBYPHISH_LOG: Blocking inline image (cid:) - attrName="${attributeName}", cid="${attributeValue}"`)
				...
				// Original placeholder logic for non-blocked content
				if (config.usePlaceholderForInlineImages) {
	...
	private readonly replaceExternalAttributesRule: Rule = {
		handler: (element, attributeName, attributeValue, config) => {
			if (config.blockExternalContent && isExternalUrl(attributeValue) && !attributeName.startsWith("draft-")) {
				this.externalContent++
				// YOUR logging
				console.log(`🔒 MOBYPHISH_LOG: Blocking external content - attrName="${attributeName}", value="${attributeValue.substring(0, 100)}"`)
