...
import { downcast, errorToString, neverNull, typedKeys, uint8ArrayToString } from "@tutao/tutanota-utils";
import { locator } from "../api/main/CommonLocator";
import { AccountType, Keys } from "../api/common/TutanotaConstants";
 ...
 */
export async function showErrorNotification(e) {
    ...
    const dialogContent = {
        view: () => {
            return [
                ...
                m(".flex.flex-column.space-around.items-center", logs.map((l) => m(BubbleButton, {
                    label: lang.makeTranslation("filename", l.name),
                    onclick: () => showLogDialog(l.name, uint8ArrayToString("utf-8", l.data)),
                }))),
                m(".flex-end", m(".right", m(ExpanderButton, {
                    label: "details_label",
...
export async function showErrorDialogNotLoggedIn(e) {
    ...
    const info = () => [
        m(".flex.col.items-end.plr", {
            ...
            m("div.mr-negative-xs", m(ExpanderButton, {
                expanded: expanded(),
        ...
        m(ExpanderPanel, {
            ...
            m(".flex-end.plr", m(Button, {
                label: "copy_action",
                click: () => copyToClipboard(message),
                type: "secondary" /* ButtonType.Secondary */,
            })),
            m(".plr.selectable.pb.scroll.text-pre", {
                style: {
                    height: px(200),
                },
            }, message),
...
export async function sendFeedbackMail(content) {
    const name = "";
...
export function clientInfoString(timestamp, loggedIn) {
    const type = loggedIn
...
export async function getLogAttachments(timestamp) {
    const logs = [];
