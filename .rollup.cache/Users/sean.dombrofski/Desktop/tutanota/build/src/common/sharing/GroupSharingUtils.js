...
export function sendShareNotificationEmail(sharedGroupInfo, recipients, texts) {
    locator.mailboxModel.getUserMailboxDetails().then((mailboxDetails) => {
        ...
        _sendNotificationEmail({
            bcc,
        }, texts.shareEmailSubject, texts.shareEmailBody(senderDisplayName, getSharedGroupName(sharedGroupInfo, locator.logins.getUserController(), true)), senderMailAddress);
    });
}
export function sendAcceptNotificationEmail(invitation, texts) {
    const to = [
        {
...
export function sendRejectNotificationEmail(invitation, texts) {
    const to = [
        {
            name: invitation.inviterName,
            address: invitation.inviterMailAddress,
...
function _sendNotificationEmail(recipients, subject, body, senderMailAddress) {
    import("../misc/HtmlSanitizer").then(({ htmlSanitizer }) => {
        ...
        locator.mailboxModel.getUserMailboxDetails().then(async (mailboxDetails) => {
            ...
            await model.send("0" /* MailMethod.NONE */, confirm, wait, "tooManyMailsAuto_msg");
        });
    });
}
//# sourceMappingURL=GroupSharingUtils.js.map