import { concat, stringToUtf8Uint8Array, uint8ArrayToBase64, utf8Uint8ArrayToString } from "@tutao/tutanota-utils";
...
export function decryptSearchIndexEntry(key, entry, dbIv) {
...
export function encryptMetaData(key, metaData) {
...
export function decryptMetaData(key, encryptedMeta) {
...
export function getAttributeIds(model) {
...
export function typeRefToTypeInfo(typeRef) {
    // @ts-ignore
...
export function userIsGlobalAdmin(user) {
    return user.memberships.some((m) => m.groupType === GroupType.Admin);
}
export function filterIndexMemberships(user) {
...
export function filterMailMemberships(user) {
    return user.memberships.filter((m) => m.groupType === GroupType.Mail);
}
export function _createNewIndexUpdate(typeInfo) {
    return {
        typeInfo,
        create: {
