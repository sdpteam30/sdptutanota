import { createAdminGroupKeyDistributionElement, createAdminGroupKeyRotationPostIn, createAdminGroupKeyRotationPutIn, createGroupKeyRotationData, createGroupKeyRotationPostIn, createGroupKeyUpdateData, createGroupMembershipKeyData, createGroupMembershipUpdateData, createKeyMac, createKeyPair, createMembershipPutIn, createPubEncKeyData, createRecoverCodeData, createUserGroupKeyRotationData, createUserGroupKeyRotationPostIn, CustomerTypeRef, GroupInfoTypeRef, GroupKeyUpdateTypeRef, GroupMemberTypeRef, GroupTypeRef, KeyRotationTypeRef, SentGroupInvitationTypeRef, UserGroupRootTypeRef, UserTypeRef, } from "../../entities/sys/TypeRefs.js";
import { asPublicKeyIdentifier, assertEnumValue, CryptoProtocolVersion, GroupKeyRotationType, GroupType, PublicKeyIdentifierType, } from "../../common/TutanotaConstants.js";
import { assertNotNull, defer, downcast, getFirstOrThrow, groupBy, isEmpty, isNotNull, isSameTypeRef, promiseMap, } from "@tutao/tutanota-utils";
...
import { bitArrayToUint8Array, createAuthVerifier, getKeyLengthBytes, isEncryptedPqKeyPairs, KEY_LENGTH_BYTES_AES_256, uint8ArrayToKey, } from "@tutao/tutanota-crypto";
import { AdminGroupKeyRotationService, GroupKeyRotationInfoService, GroupKeyRotationService, MembershipService, UserGroupKeyRotationService, } from "../../entities/sys/Services.js";
import { assertWorkerOrNode } from "../../common/Env.js";
import { getUserGroupMemberships } from "../../common/utils/GroupUtils.js";
import { InternalRecipientKeyDataTypeRef } from "../../entities/tutanota/TypeRefs.js";
...
import { asPQPublicKeys, brandKeyMac } from "./KeyAuthenticationFacade.js";
assertWorkerOrNode();
export var MultiAdminGroupKeyAdminActionPath;
(function (MultiAdminGroupKeyAdminActionPath) {
    MultiAdminGroupKeyAdminActionPath[MultiAdminGroupKeyAdminActionPath["WAIT_FOR_OTHER_ADMINS"] = 0] = "WAIT_FOR_OTHER_ADMINS";
    MultiAdminGroupKeyAdminActionPath[MultiAdminGroupKeyAdminActionPath["CREATE_DISTRIBUTION_KEYS"] = 1] = "CREATE_DISTRIBUTION_KEYS";
    MultiAdminGroupKeyAdminActionPath[MultiAdminGroupKeyAdminActionPath["PERFORM_KEY_ROTATION"] = 2] = "PERFORM_KEY_ROTATION";
    MultiAdminGroupKeyAdminActionPath[MultiAdminGroupKeyAdminActionPath["IMPOSSIBLE_STATE"] = 3] = "IMPOSSIBLE_STATE";
})(MultiAdminGroupKeyAdminActionPath || (MultiAdminGroupKeyAdminActionPath = {}));
/**
...
export class KeyRotationFacade {
    ...
    pendingGroupKeyUpdateIds; // already rotated groups for which we need to update the memberships (GroupKeyUpdateIds all in one list)
    ...
    async generatePubKeyTagsForNonAdminUsers(newAdminPubKey, newAdminGroupKeyVersion, adminGroupId, customerId, groupToExclude) {
        ...
        for (const userGroupInfo of userGroupInfos) {
            if (isSameId(userGroupInfo.group, groupToExclude))
                continue;
    ...
    async rotateUserGroupKey(user, pwKey, userGroupKeyRotation) {
        ...
        await this.serviceExecutor.post(UserGroupKeyRotationService, createUserGroupKeyRotationPostIn({
            userGroupKeyData,
        }));
    ...
    async handleUserGroupKeyRotationAsUser(userGroupKeyRotation, currentUserGroupKey, userGroupId, adminGroupId, newUserGroupKeys) {
        ...
        if (parseKeyVersion(taggingKeyVersion) !== currentUserGroupKey.version) {
            throw new Error(`the encrypting key version in the userEncAdminPubKeyHash does not match hash: ${taggingKeyVersion} current user group key:${currentUserGroupKey.version}`);
        }
    ...
    async rotateMultipleAdminsGroupKeys(user, passphraseKey, keyRotation) {
        ...
        switch (this.decideMultiAdminGroupKeyRotationNextPathOfAction(userGroupIdsMissingDistributionKeys, user, distributionKeys)) {
            ...
            case MultiAdminGroupKeyAdminActionPath.IMPOSSIBLE_STATE:
                throw new TutanotaError("MultiAdminGroupKeyAdminActionPathImpossibleStateMetError", "Impossible state met while performing multi admin key rotation");
        }
    }
    async performMultiAdminKeyRotation(keyRotation, user, passphraseKey, distributionKeys) {
        ...
        for (const distributionKey of distributionKeys) {
            ...
            if (isSameId(distributionKey.userGroupId, user.userGroup.group))
                continue;
...
//# sourceMappingURL=KeyRotationFacade.js.map