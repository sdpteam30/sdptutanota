...
import { getLocalisedCategoryName, getLocalisedTopicIssue } from "../SupportDialog.js";
...
import { getColors } from "../../gui/base/Button.js";
...
export class ContactSupportPage {
    ...
    view({ attrs: { data, goToSuccessPage } }) {
        return m(".flex.flex-column.pt.height-100p.gap-vpad", m(Card, m("", m("p.h4.m-0", lang.get("supportForm_title")), m("p.m-0.mt-s", lang.get("supportForm_msg")))), m(Card, {
            classes: ["child-text-editor", "rel", "height-100p"],
            style: {
                padding: "0",
            },
        }, m(data.htmlEditor)), m(".flex.flex-column.gap-vpad.pb", {
            style: {
                marginTop: "auto",
            },
        }, m(Card, {
            ...
            m(SectionButton, {
                ...
                rightIcon: { icon: "Attachment" /* Icons.Attachment */, title: "attachFiles_action" },
                onclick: async (_, dom) => {
                    await chooseAndAttachFile(this.sendMailModel, dom.getBoundingClientRect());
                    m.redraw();
                },
            }),
            (this.sendMailModel?.getAttachments() ?? []).map((attachment) => m(".flex.center-vertically.flex-space-between.pb-s.pt-s", { style: { paddingInline: px(size.vpad_small) } }, m("span.smaller", attachment.name), m(BaseButton, {
                label: "remove_action",
                onclick: () => {
        ...
        ]), m(Card, m(".flex.gap-vpad-s.items-center", [
            m(Switch, {
        ...
        ])), m(".align-self-center.full-width", m(LoginButton, {
            label: "send_action",
            onclick: async () => {
                ...
                await this.sendMailModel.send("0" /* MailMethod.NONE */, () => Promise.resolve(true), showProgressDialog);
                goToSuccessPage();
            },
        }))));
    }
