import { AccessBlockedError, AccessDeactivatedError, AccessExpiredError, ConnectionError, InsufficientStorageError, InvalidSoftwareVersionError, NotAuthenticatedError, RequestTimeoutError, ServiceUnavailableError, SessionExpiredError, } from "../api/common/error/RestError";
...
export async function handleUncaughtErrorImpl(e) {
    ...
    if (isOfflineError(e)) {
        ...
        const isOffline = isOfflineStorageAvailable() && logins.isUserLoggedIn() && logins.getUserController().sessionType === 2 /* SessionType.Persistent */;
...
export async function reloginForExpiredSession() {
    ...
    const dialog = showRequestPasswordDialog({
        action: async (pw) => {
            ...
            if (oldSessionType === 2 /* SessionType.Persistent */) {
