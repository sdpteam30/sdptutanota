...
export class DesktopUtils {
    ...
    async cleanupOldInstance() {
        if (!(await this.makeSingleInstance()))
            return false;
    ...
    async unregisterAsMailtoHandler() {
     ...
     * create a new window.
     */
    async handleSecondInstance(wm, args) {
     ...
     * @private
     */
    async executeRegistryScript(script) {
    ...
    async doRegisterMailtoOnWin32WithCurrentUser() {
    ...
    async doUnregisterMailtoOnWin32WithCurrentUser() {
        if (process.platform !== "win32") {
            throw new ProgrammingError("Not win32");
        }
    ...
    async _openDefaultAppsSettings() {
        try {
    ...
    async handleMailto(wm, mailToArg = this.mailtoArg) {
        if (mailToArg) {
...
export function isRectContainedInRect(closestRect, lastBounds) {
...
function findMailToUrlInArgv(argv) {
