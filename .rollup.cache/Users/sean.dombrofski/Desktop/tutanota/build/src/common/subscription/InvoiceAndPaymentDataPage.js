...
import { getClientType, getDefaultPaymentMethod, Keys, PaymentMethodTypeToName, } from "../api/common/TutanotaConstants";
...
import { getLazyLoadedPayPalUrl, getPreconditionFailedPaymentMsg } from "./SubscriptionUtils";
 ...
 */
export class InvoiceAndPaymentDataPage {
    ...
    oncreate(vnode) {
        ...
        if (!locator.logins.isUserLoggedIn()) {
            login = locator.logins
                .createSession(neverNull(data.newAccountData).mailAddress, neverNull(data.newAccountData).password, 1 /* SessionType.Temporary */)
    ...
    view(vnode) {
        ...
        const onNextClick = () => {
            ...
            if (error) {
                ...
                return showProgressDialog("updatePaymentDataBusy_msg", Promise.resolve()
                    .then(() => {
                    ...
                    .then(() => updatePaymentData(a.data.options.paymentInterval(), a.data.invoiceData, a.data.paymentData, null, a.data.upgradeType === "Signup" /* UpgradeType.Signup */, neverNull(a.data.price?.rawPrice), neverNull(a.data.accountingInfo)).then((success) => {
                    if (success) {
        ...
        return m(".pt", this._availablePaymentMethods
            ? [
                m(SegmentControl, {
...
export class InvoiceAndPaymentDataPageAttrs {
    data;
    _enabled = () => true;
...
export async function updatePaymentData(paymentInterval, invoiceData, paymentData, confirmedCountry, isSignup, price, accountingInfo) {
    ...
    if (statusCode === "0" /* PaymentDataResultType.OK */) {
    ...
    else if (statusCode === "1" /* PaymentDataResultType.COUNTRY_MISMATCH */) {
    ...
    else if (statusCode === "2" /* PaymentDataResultType.INVALID_VATID_NUMBER */) {
        await Dialog.message(lang.makeTranslation("invalidVatIdNumber_msg", lang.get("invalidVatIdNumber_msg") + (isSignup ? " " + lang.get("accountWasStillCreated_msg") : "")));
    }
    else if (statusCode === "3" /* PaymentDataResultType.CREDIT_CARD_DECLINED */) {
        await Dialog.message(lang.makeTranslation("creditCardDeclined_msg", lang.get("creditCardDeclined_msg") + (isSignup ? " " + lang.get("accountWasStillCreated_msg") : "")));
    }
        ...
        await Dialog.message(lang.makeTranslation("paymentProviderNotAvailableError_msg", lang.get("paymentProviderNotAvailableError_msg") + (isSignup ? " " + lang.get("accountWasStillCreated_msg") : "")));
    }
    else if (statusCode === "7" /* PaymentDataResultType.OTHER_PAYMENT_ACCOUNT_REJECTED */) {
        await Dialog.message(lang.makeTranslation("paymentAccountRejected_msg", lang.get("paymentAccountRejected_msg") + (isSignup ? " " + lang.get("accountWasStillCreated_msg") : "")));
    }
        ...
        await Dialog.message(lang.makeTranslation("creditCardNumberInvalid_msg", lang.get("creditCardNumberInvalid_msg") + (isSignup ? " " + lang.get("accountWasStillCreated_msg") : "")));
    }
    else if (statusCode === "8" /* PaymentDataResultType.COULD_NOT_VERIFY_VATID */) {
        await Dialog.message(lang.makeTranslation("invalidVatIdValidationFailed_msg", lang.get("invalidVatIdValidationFailed_msg") + (isSignup ? " " + lang.get("accountWasStillCreated_msg") : "")));
    }
    else if (statusCode === "11" /* PaymentDataResultType.CREDIT_CARD_VERIFICATION_LIMIT_REACHED */) {
        await Dialog.message(lang.makeTranslation("creditCardVerificationLimitReached_msg", lang.get("creditCardVerificationLimitReached_msg") + (isSignup ? " " + lang.get("accountWasStillCreated_msg") : "")));
    }
    else {
        await Dialog.message(lang.makeTranslation("otherPaymentProviderError_msg", lang.get("otherPaymentProviderError_msg") + (isSignup ? " " + lang.get("accountWasStillCreated_msg") : "")));
    }
    return false;
}
/**
...
//# sourceMappingURL=InvoiceAndPaymentDataPage.js.map