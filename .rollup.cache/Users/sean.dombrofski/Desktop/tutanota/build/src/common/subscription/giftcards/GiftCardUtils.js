...
import { CURRENT_GIFT_CARD_TERMS_VERSION, renderTermsAndConditionsButton } from "../TermsAndConditions";
...
export function loadGiftCards(customerId) {
...
export async function generateGiftCardLink(giftCard) {
...
export function showGiftCardToShare(giftCard) {
    generateGiftCardLink(giftCard).then((link) => {
        ...
        const dialog = Dialog.largeDialog({
            ...
            view: () => [
                m(".flex-center.full-width.pt.pb", m(".pt-l", // Needed to center SVG
                {
                    style: {
                        width: "480px",
                    },
                ...
                m(".flex-center", [
                    m(IconButton, {
                        click: () => {
                            dialog.close();
                            setTimeout(() => import("../../../mail-app/mail/editor/MailEditor").then((editor) => editor.writeGiftCardMail(link)), DefaultAnimationTime);
                        },
                    ...
                    isAndroidApp()
                        ? m(IconButton, {
                            click: () => {
                                locator.systemFacade.shareText(lang.get("nativeShareGiftCard_msg", {
                                    "{link}": link,
                                }), lang.get("nativeShareGiftCard_label"));
                            },
                            title: "share_action",
                            icon: "Share" /* BootIcons.Share */,
                        })
                        : m(IconButton, {
                            click: () => {
...
export function renderGiftCardSvg(price, link, message) {
...
export function renderAcceptGiftCardTermsCheckbox(checked, onChecked, classes) {
    return m(Checkbox, {
        checked,
        onChecked,
        class: classes,
        label: () => [lang.get("termsAndConditions_label"), m("div", renderTermsAndConditionsButton("giftCardsTerms-entries" /* TermsSection.GiftCards */, CURRENT_GIFT_CARD_TERMS_VERSION))],
    });
}
//# sourceMappingURL=GiftCardUtils.js.map