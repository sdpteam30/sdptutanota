...
export class InfoBanner {
    view(vnode) {
        ...
        const buttonContainerStyle = helpLink != null || buttons.length > 0
            ...
            : undefined;
        return m(".center-vertically.border-bottom.pr-s.pl.border-radius.mt-xs", {
            style: {
                border: `solid 2px ${type === "warning" /* BannerType.Warning */ ? WARNING_RED : theme.content_border}`,
            ...
            m("", { style: { "margin-left": px(size.icon_size_large + 1) } }, // allow room for the icon
            [
    ...
    renderIcon(icon, type) {
        return m(Icon, {
            icon,
            style: {
                fill: type === "warning" /* BannerType.Warning */ ? WARNING_RED : theme.content_button,
                display: "block",
            },
        });
    }
    renderButtons(buttons) {
        if (buttons.length === 0)
            return null;
        return m(".small.flex.row", buttons.filter(isNotNull).map((attrs) => m(Button, { ...attrs, type: "secondary" /* ButtonType.Secondary */ })));
    }
    renderHelpLink(helpLink) {
        ...
        return [
            ...
            m(".button-content", {
                ...
                icon: () => "QuestionMark" /* Icons.QuestionMark */,
                href: helpLink,
                small: true,
                hideLabel: true,
                centred: true,
