...
import { EventImportRejectionReason, sortOutParsedEvents } from "./ImportExportUtils.js";
/**
...
async function partialImportConfirmation(skippedEvents, confirmationText, total) {
    return (skippedEvents.length === 0 ||
        (await Dialog.confirm(lang.makeTranslation("confirm_msg", lang.get(confirmationText, {
            "{amount}": skippedEvents.length + "",
            "{total}": total + "",
        })))));
}
export async function handleCalendarImport(calendarGroupRoot, importedParsedEvents = null, calendarType = CalendarType.NORMAL) {
    ...
    if (!(await partialImportConfirmation(rejectedEvents.get(EventImportRejectionReason.Duplicate) ?? [], "importEventExistingUid_msg", total)))
        return;
    if (!(await partialImportConfirmation(rejectedEvents.get(EventImportRejectionReason.InvalidDate) ?? [], "importInvalidDatesInEvent_msg", total)))
        return;
    if (!(await partialImportConfirmation(rejectedEvents.get(EventImportRejectionReason.Inversed) ?? [], "importEndNotAfterStartInEvent_msg", total)))
        return;
    if (!(await partialImportConfirmation(rejectedEvents.get(EventImportRejectionReason.Pre1970) ?? [], "importPre1970StartInEvent_msg", total)))
        return;
    if (eventsForCreation.length > 0) {
        if (isExternalCalendarType(calendarType))
            await importEvents(eventsForCreation);
        else
            showEventsImportDialog(eventsForCreation.map((ev) => ev.event), async (dialog) => {
                dialog.close();
                await importEvents(eventsForCreation);
            }, "importEvents_label");
    }
...
async function selectAndParseIcalFile() {
    try {
        ...
        if (e instanceof ParserError) {
            ...
            Dialog.message(lang.makeTranslation("confirm_msg", lang.get("importReadFileError_msg", {
                "{filename}": e.filename ?? "",
            })));
            return [];
        }
...
async function importEvents(eventsForCreation) {
    ...
    return showProgressDialog("importCalendar_label", locator.calendarFacade.saveImportedCalendarEvents(eventsForCreation, operation.id), operation.progress)
        .catch(ofClass(ImportError, (e) => Dialog.message(lang.makeTranslation("confirm_msg", lang.get("importEventsError_msg", {
        "{amount}": e.numFailed + "",
...
export async function exportCalendar(calendarName, groupRoot, userAlarmInfos, now, zone) {
    return await showProgressDialog("pleaseWait_msg", (async () => {
        const allEvents = await loadAllEvents(groupRoot);
        const eventsWithAlarms = await promiseMap(allEvents, async (event) => {
...
//# sourceMappingURL=CalendarImporterDialog.js.map