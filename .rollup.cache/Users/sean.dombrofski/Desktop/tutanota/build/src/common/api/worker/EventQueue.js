 ...
 */
export function batchMod(batchId, batch, entityUpdate) {
    for (const batchEvent of batch) {
        if (entityUpdate.instanceId === batchEvent.instanceId &&
...
function lastOperationKey(update) {
    ...
    if (update.instanceListId) {
        return `${typeIdentifier}/${update.instanceListId}/${update.instanceId}`;
    }
    else {
        return `${typeIdentifier}/${update.instanceId}`;
    }
}
export class EventQueue {
                ...
                if (newEntityModification === "UPDATE" /* EntityModificationType.UPDATE */) {
                    switch (lastEntityModification) {
                        ...
                        case "DELETE" /* EntityModificationType.DELETE */:
                            throw new ProgrammingError(`UPDATE not allowed after DELETE. Last batch: ${lastBatchForEntity.batchId}, new batch: ${batchId}, ${newEvent.type} ${lastOpKey}`);
                    }
                }
    ...
    removeEventsForInstance(operationKey, startIndex = 0) {
        ...
        for (let i = startIndex; i < this.eventQueue.length; i++) {
            ...
            findAllAndRemove(batchInThePast.events, (event) => event.operation !== "2" /* OperationType.DELETE */ && lastOperationKey(event) === operationKey);
        }
    }
    start() {
