import m from "mithril";
import { landmarkAttrs } from "../AriaUtils.js";
import { inputLineHeight, px, size } from "../size.js";
import { styles } from "../styles.js";
import { DefaultAnimationTime } from "../animation/Animations.js";
import { Icon, IconSize } from "./Icon.js";
import { theme } from "../theme.js";
import { BaseButton } from "./buttons/BaseButton.js";
import { getOperatingClasses } from "./GuiUtils.js";
export class BaseSearchBar {
    domInput;
    isFocused = false;
    view({ attrs }) {
        return m(".flex-end.items-center.border-radius.plr-s.pt-xs.pb-xs.search-bar.flex-grow.click", {
            focused: String(this.isFocused),
            ...landmarkAttrs("search" /* AriaLandmarks.Search */),
            class: getOperatingClasses(attrs.disabled),
            style: {
                "min-height": px(inputLineHeight + 2),
                "margin-top": px(6),
                "margin-bottom": px(6),
                "max-width": styles.isUsingBottomNavigation() ? "" : px(350),
            },
            oncreate: ({ dom }) => {
                attrs.onWrapperCreated?.(dom);
            },
            onclick: () => {
                if (!attrs.disabled) {
                    this.domInput?.focus();
                    attrs.onSearchClick?.();
                }
            },
        }, [
            styles.isDesktopLayout()
                ? m(Icon, {
                    icon: "Search" /* BootIcons.Search */,
                    size: IconSize.Medium,
                    style: {
                        fill: theme.content_button,
                    },
                })
                : null,
            m(".flex.items-center", {
                style: {
                    width: "100%",
                    transition: `width ${DefaultAnimationTime}ms`,
                    "padding-left": styles.isDesktopLayout() ? px(10) : px(6),
                    "padding-top": "3px",
                    "padding-bottom": "3px",
                    "overflow-x": "hidden",
                },
            }, [this.renderInputField(attrs)]),
            attrs.busy || attrs.text
                ? m(BaseButton, {
                    label: attrs.busy ? "loading_msg" : "close_alt",
                    icon: m(Icon, {
                        container: "div",
                        size: IconSize.Medium,
                        icon: attrs.busy ? "Progress" /* BootIcons.Progress */ : "Close" /* Icons.Close */,
                        class: "center-h  " + (attrs.busy ? "icon-progress-search icon-progress" : ""),
                        style: {
                            fill: theme.header_button,
                        },
                    }),
                    onclick: () => {
                        if (!attrs.disabled)
                            attrs.onClear?.();
                    },
                    disabled: attrs.busy,
                    style: {
                        width: size.icon_size_large,
                    },
                })
                : null,
        ]);
    }
    renderInputField(attrs) {
        return m("input.input.input-no-clear", {
            "aria-autocomplete": "list",
            tabindex: "0" /* TabIndex.Default */,
            role: "combobox",
            placeholder: attrs.placeholder,
            type: "text" /* TextFieldType.Text */,
            value: attrs.text,
            disabled: attrs.disabled,
            oncreate: (vnode) => {
                this.domInput = vnode.dom;
                attrs.onInputCreated?.(this.domInput);
            },
            onfocus: () => {
                this.isFocused = true;
                attrs.onFocus?.();
            },
            onblur: () => {
                this.isFocused = false;
                attrs.onBlur?.();
            },
            onremove: () => {
                this.domInput.onblur = null;
                this.isFocused = false;
            },
            oninput: () => {
                attrs.onInput(this.domInput.value);
            },
            onkeydown: (e) => {
                attrs.onKeyDown?.(e);
            },
            style: {
                "line-height": px(inputLineHeight),
            },
        });
    }
}
//# sourceMappingURL=BaseSearchBar.js.map