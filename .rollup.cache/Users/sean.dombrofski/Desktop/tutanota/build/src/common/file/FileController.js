...
export var VCARD_MIME_TYPES;
(function (VCARD_MIME_TYPES) {
    VCARD_MIME_TYPES["X_VCARD"] = "text/x-vcard";
    VCARD_MIME_TYPES["VCARD"] = "text/vcard";
})(VCARD_MIME_TYPES || (VCARD_MIME_TYPES = {}));
export var MAIL_MIME_TYPES;
(function (MAIL_MIME_TYPES) {
    MAIL_MIME_TYPES["EML"] = "message/rfc822";
    MAIL_MIME_TYPES["MBOX"] = "application/mbox";
})(MAIL_MIME_TYPES || (MAIL_MIME_TYPES = {}));
/**
...
export class FileController {
        ...
        try {
            ...
            for (const file of tutanotaFiles) {
                try {
                    ...
                    if (isOffline)
                        break; // don't try to download more files, but the previous ones (if any) will still be downloaded
                }
            }
            if (downloadedFiles.length > 0) {
                if (action === 0 /* DownloadPostProcessing.Open */) {
     ...
     */
    async download(file) {
        await this.observeProgress(this.doDownload([file], 1 /* DownloadPostProcessing.Write */));
    }
    /**
    ...
    async downloadAll(files) {
        ...
        await this.observeProgress(this.doDownload(files, 1 /* DownloadPostProcessing.Write */, progress), progress);
    }
    /**
     ...
     */
    async open(file) {
        await this.observeProgress(this.doDownload([file], 0 /* DownloadPostProcessing.Open */));
    }
}
export function handleDownloadErrors(e, errorAction) {
    if (isOfflineError(e)) {
...
export function readLocalFiles(nativeFiles) {
 ...
 */
export function showFileChooser(allowMultiple, allowedExtensions) {
 ...
 */
export async function zipDataFiles(dataFiles, name) {
...
export async function openDataFileInBrowser(dataFile) {
    try {
        ...
        const needsPdfWorkaround = dataFile.mimeType === "application/pdf" && client.browser === "Firefox" /* BrowserType.FIREFOX */ && client.browserVersion >= 98;
        ...
        if (typeof a.download !== "undefined") {
            ...
            if (client.isIos() && client.browser === "Chrome" /* BrowserType.CHROME */ && typeof FileReader === "function") {
...
export async function downloadAndDecryptDataFile(file, blobFacade) {
    const bytes = await blobFacade.downloadAndDecrypt("1" /* ArchiveDataType.Attachments */, createReferencingInstance(file));
    return convertToDataFile(file, bytes);
}
export async function guiDownload(downloadPromise, progress) {
    try {
...
export async function showNativeFilePicker(fileTypes, isFileOnly = false) {
    if (isApp()) {
        const rect = { width: 0, height: 0, left: 0, top: 0 };
        try {
