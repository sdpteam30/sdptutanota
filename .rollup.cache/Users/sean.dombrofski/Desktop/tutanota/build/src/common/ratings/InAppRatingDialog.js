 ...
 */
export async function showAppRatingDialog() {
    const selectedValue = await new Promise((resolve) => {
        ...
        const headerBarProps = {
            right: [
                {
                    label: "notNow_label",
                    click: () => choose("notNow" /* AppRatingValue.NotNow */),
        ...
        const dialog = new Dialog("EditSmall" /* DialogType.EditSmall */, {
            view: () => m("", [
                m(".dialog-header.plr-l.flex-space-between.dialog-header-line-height", [
                    ...
                    m(columnClass + ".mr-negative-s.flex.justify-end", resolveMaybeLazy(neverNull(headerBarProps.right)).map((a) => m(Button, a))),
                ]),
                m(".plr-l.pb-ml.text-break", m(".flex.flex-column", [
                    m("#dialog-message.dialog-max-height.text-break.text-prewrap.selectable.scroll", m("", m(".flex-center.mt-m", m("img.pb.pt.block.height-100p", {
                    ...
                    })), m("h1.text-center", lang.get("ratingHowAreWeDoing_title")), m("p.text-center", lang.get("ratingExplanation_msg")))),
                    m(".flex.flex-column.mt", { style: { gap: "1em" } }, m(BaseButton, {
                        label: "ratingLoveIt_label",
                        text: lang.get("ratingLoveIt_label"),
                        onclick: () => choose("happy" /* AppRatingValue.Happy */),
                        ...
                        onclick: () => choose("unhappy" /* AppRatingValue.Unhappy */),
            ...
            help: "close_alt",
            key: Keys.ESC,
            exec: () => choose("notNow" /* AppRatingValue.NotNow */),
        });
        dialog.show();
    });
...
//# sourceMappingURL=InAppRatingDialog.js.map