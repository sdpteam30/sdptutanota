...
export class WebDialog {
    ...
    closedPromise;
    browserWindow;
    constructor(facade, closedPromise, browserWindow) {
        this.facade = facade;
...
export class WebDialogController {
    async create(parentWindowId, urlToOpen) {
        ...
        const facadePromise = this.initRemoteFacade(bw.webContents);
    ...
    async createBrowserWindow(parentWindowId) {
     ...
     */
    async initRemoteFacade(webContents) {
        const deferred = defer();
        const transport = new ElectronWebContentsTransport(webContents, webauthnIpcConfig);
        const dispatcher = new MessageDispatcher(transport, {
            init: () => {
                deferred.resolve();
                return Promise.resolve();
            },
        }, "node-webauthn");
        const facade = exposeRemote((req) => dispatcher.postRequest(req));
        await deferred.promise;
        return facade;
    }
}
//# sourceMappingURL=WebDialog.js.map