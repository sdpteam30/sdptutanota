...
import { lang, languages } from "../misc/LanguageViewModel.js";
...
export function showAddOrEditNotificationEmailDialog(userController, selectedNotificationLanguage) {
...
export async function showBuyOrSetNotificationEmailDialog(lastBooking, customerProperties, existingTemplate) {
...
export function show(existingTemplate, customerProperties) {
    let template;
    if (!existingTemplate) {
    ...
    const selectedLanguageStream = stream(selectedLanguage.value);
    ...
    Dialog.showActionDialog({
        ...
        okAction: (dialog) => {
            if (!editor.getValue().includes("{link}")) {
                return Dialog.message(lang.getTranslation("templateMustContain_msg", {
                    "{value}": "{link}",
                }));
            }
            let templates;
...
function getDefaultNotificationMail() {
    return (HTML_PTAG_START +
        ...
        lang.get("externalNotificationMailBody2_msg", {
            "{1}": "https://tuta.com" /* InfoLink.HomePage */,
        }) +
        HTML_PTAG_END +
        HTML_PTAG_START +
        "<a href='{link}'>" +
...
//# sourceMappingURL=EditNotificationEmailDialog.js.map