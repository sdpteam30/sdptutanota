...
import { getDesktopIntegratorForPlatform } from "./integration/DesktopIntegrator";
...
import { exposeLocalDelayed } from "../api/common/WorkerProxy.js";
...
const loadArgon2 = async () => {
    ...
    const wasmSource = await fs.promises.readFile(wasmSourcePath);
...
async function createComponents() {
    ...
    const offlineDbFactory = {
        async create(userId, key, retry = true) {
    ...
    conf.on(DesktopConfigKey.runAsTrayApp, async (value) => {
        if (value) {
            await tray.create();
    ...
    const notificationHandler = new TutaNotificationHandler(wm, nativeCredentialsFacade, sseStorage, notifier, desktopAlarmScheduler, alarmStorage, lang, suspensionAwareFetch, app.getVersion());
    ...
    const dispatcherFactory = (window) => {
        // @ts-ignore
        ...
        const windowCleanup = {
            async onCleanup(userId) {
                desktopExportLock.unlock(userId);
                log.debug(TAG, `closing offline db for userId ${userId}`);
                await sqlCipherFacade.closeDb();
            },
        };
        const dispatcher = new DesktopGlobalDispatcher(desktopCommonSystemFacade, new DesktopDesktopSystemFacade(wm, window, sock), new DesktopExportFacade(tfs, electron, conf, window, dragIcons, mailboxExportPersistence, fs, dateProvider, desktopExportLock), new DesktopExternalCalendarFacade(), new DesktopFileFacade(window, conf, dateProvider, customFetch, electron, tfs, fs), new DesktopInterWindowEventFacade(window, wm), nativeCredentialsFacade, desktopCrypto, desktopImportFacade, pushFacade, new DesktopSearchTextInAppFacade(window), settingsFacade, sqlCipherFacade, themeFacade, new DesktopWebauthnFacade(window, webDialogController));
        return { desktopCommonSystemFacade, windowCleanup, dispatcher };
    };
    const facadeHandlerFactory = (window) => {
        return exposeLocalDelayed({
...
async function startupInstance(components) {
    const { wm, sse, tfs } = components;
    if (!(await desktopUtils.cleanupOldInstance()))
        return;
...
async function onAppReady(components) {
...
function manageDownloadsForSession(session, dictUrl) {
