...
import { GENERATION_NUMBER, NEW_LINE, PDF_DEFAULT_OBJECTS, PDF_METADATA, PdfStreamEncoding } from "./PdfConstants.js";
...
export class PdfWriter {
    ...
    makeXRefTable() {
        ...
        for (const pdfObject of this.pdfObjectList) {
            if (pdfObject.getBytePosition() === -1)
                throw new Error(`Found an object with invalid byte-position! ${pdfObject.getObjectNumber()}`);
    ...
    async setupResourceObjects() {
        ...
        if (!this.cachedResources) {
            this.cachedResources = await Promise.all(["/pdf/SourceSans3-Regular.ttf", "/pdf/SourceSans3-Bold.ttf", "/pdf/sRGB2014.icc", "/pdf/identity_h.cmap", "/pdf/tutanota_logo_en.jpg"].map((url) => typeof this.customFetch !== "undefined"
        ...
        this.createStreamObject(new Map([
            ...
            ["N", "3"],
        ]), await this.deflater.deflate(colorProfile), PdfStreamEncoding.FLATE, "DEST_OUTPUT_PROFILE");
        ...
        this.createStreamObject(new Map([
            ["Type", "/Metadata"],
            ["Subtype", "/XML"],
        ]), new Uint8Array(this.textEncoder.encode(metaData)), PdfStreamEncoding.NONE, "METADATA");
    }
    /**
