...
export async function showSpellcheckLanguageDialog() {
...
export async function getCurrentSpellcheckLanguageLabel() {
    const current = await getCurrentSpellcheckLanguage();
    const items = await getItems();
...
async function getItems() {
    ...
    return [
        ...
        ...options
            ...
            const language = languages.find((language) => locale && language.code === `${langCode}_${locale.toLowerCase()}`) || // find the name for a language without a locale, with a perfect match
...
//# sourceMappingURL=SpellcheckLanguageDialog.js.map