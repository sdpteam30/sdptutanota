import { assertNotNull, base64ToUint8Array, downcast, isSameTypeRef, isSameTypeRefByAttr, neverNull, ofClass, promiseMap, stringToUtf8Uint8Array, TypeRef, uint8ArrayToBase64, } from "@tutao/tutanota-utils";
import { AccountType, asCryptoProtoocolVersion, assertEnumValue, CryptoProtocolVersion, EncryptionAuthStatus, GroupType, PublicKeyIdentifierType, SYSTEM_GROUP_MAIL_ADDRESS, } from "../../common/TutanotaConstants";
import { resolveTypeReference } from "../../common/EntityFunctions";
import { BucketKeyTypeRef, BucketPermissionTypeRef, createInstanceSessionKey, createUpdatePermissionKeyData, GroupInfoTypeRef, GroupTypeRef, PermissionTypeRef, PushIdentifierTypeRef, } from "../../entities/sys/TypeRefs.js";
import { ContactTypeRef, createEncryptTutanotaPropertiesData, createInternalRecipientKeyData, createSymEncInternalRecipientKeyData, FileTypeRef, MailTypeRef, TutanotaPropertiesTypeRef, } from "../../entities/tutanota/TypeRefs.js";
...
import { aes256RandomKey, aesEncrypt, bitArrayToUint8Array, decryptKey, encryptKey, isPqKeyPairs, sha256Hash, } from "@tutao/tutanota-crypto";
...
import { encryptKeyWithVersionedKey } from "./CryptoWrapper.js";
assertWorkerOrNode();
export class CryptoFacade {
    ...
    async convertBucketKeyToInstanceIfNecessary(bucketKeyInstanceOrLiteral) {
        if (this.isLiteralInstance(bucketKeyInstanceOrLiteral)) {
            ...
            return bucketKeyInstanceOrLiteral;
        }
    }
    ...
    async applyCustomerGroupOwnershipToGroupInfo(data) {
        ...
        if (!customerGroupPermission)
            throw new SessionKeyNotFoundError("Permission not found, could not apply OwnerGroup migration");
    ...
    async trySymmetricPermission(listPermissions) {
        const symmetricPermission = listPermissions.find((p) => (p.type === "2" /* PermissionType.Public_Symmetric */ || p.type === "1" /* PermissionType.Symmetric */) &&
            p._ownerGroup &&
            this.userFacade.hasGroup(p._ownerGroup)) ?? null;
            ...
            if (!encryptionAuthStatus) {
                if (!pqMessageSenderKey) {
                    ...
                    const mail = this.isLiteralInstance(instance)
                        ? (await this.instanceMapper.decryptAndMapToInstance(typeModel, instance, resolvedSessionKeyForInstance))
                        : instance;
                    const senderMailAddress = mail.confidential ? mail.sender.address : SYSTEM_GROUP_MAIL_ADDRESS;
                    encryptionAuthStatus = await this.tryAuthenticateSenderOfMainInstance(senderMailAddress, pqMessageSenderKey, 
    ...
    async resolveWithPublicOrExternalPermission(listPermissions, instance, typeModel) {
        const pubOrExtPermission = listPermissions.find((p) => p.type === "0" /* PermissionType.Public */ || p.type === "5" /* PermissionType.External */) ?? null;
        ...
        const bucketPermission = bucketPermissions.find((bp) => (bp.type === "2" /* BucketPermissionType.Public */ || bp.type === "3" /* BucketPermissionType.External */) && pubOrExtPermission._ownerGroup === bp._ownerGroup);
        ...
        if (bucketPermission.type === "3" /* BucketPermissionType.External */) {
    ...
    async decryptWithExternalBucket(bucketPermission, pubOrExtPermission, instance) {
        ...
        if (bucketPermission.ownerEncBucketKey != null) {
            ...
            throw new SessionKeyNotFoundError(`BucketEncSessionKey is not defined for Permission ${pubOrExtPermission._id.toString()} (Instance: ${JSON.stringify(instance)})`);
        }
...
//# sourceMappingURL=CryptoFacade.js.map