...
import { alpha, animations, transform } from "../animation/Animations.js";
...
assertMainOrNode();
export const gestureInfoFromTouch = (touch) => ({
    x: touch.pageX,
    y: touch.pageY,
...
export class ViewSlider {
    ...
    handleHistoryEvent = () => {
        ...
        if (prev != null && prev.columnType !== 0 /* ColumnType.Foreground */) {
    ...
    constructor(viewColumns, enableDrawer = true) {
        ...
        this.mainColumn = assertNotNull(viewColumns.find((column) => column.columnType === 1 /* ColumnType.Background */), "there was no backgroung column passed to viewslider");
            ...
            return m(".fill-absolute.flex.col", {
                oncreate: (vnode) => {
                    if (this.enableDrawer)
                        this.attachTouchHandler(vnode.dom);
                },
                onremove: () => {
                    if (this.viewColumns[0].columnType === 0 /* ColumnType.Foreground */ && this.viewColumns[0].isInForeground) {
    ...
    getColumnRole(column) {
        ...
        return this.mainColumn === column ? "main" /* AriaLandmarks.Main */ : "region" /* AriaLandmarks.Region */;
    }
    getMainColumn() {
        return this.mainColumn;
    }
    getColumnsForMainSlider() {
        return this.viewColumns.filter((c) => c.columnType === 1 /* ColumnType.Background */ || c.isVisible);
    }
    getColumnsForOverlay() {
        return this.viewColumns.filter((c) => c.columnType === 0 /* ColumnType.Foreground */ && !c.isVisible);
    }
        ...
        if (this.isModalBackgroundVisible) {
            return [
                m(".fill-absolute.will-change-alpha", {
                    ...
                    oncreate: (vnode) => {
                        this.busy.then(() => animations.add(vnode.dom, alpha("backgroundColor" /* AlphaEnum.BackgroundColor */, theme.modal_bg, 0, 0.5)));
                    },
                    onbeforeremove: (vnode) => {
                        return this.busy.then(() => animations.add(vnode.dom, alpha("backgroundColor" /* AlphaEnum.BackgroundColor */, theme.modal_bg, 0.5, 0)));
                    },
                    onclick: () => {
    ...
    updateVisibleBackgroundColumns() {
        ...
        let visibleColumns = [this.focusedColumn.columnType === 1 /* ColumnType.Background */ ? this.focusedColumn : this.mainColumn];
    ...
    isUsingOverlayColumns() {
        return this.viewColumns.every((c) => c.columnType !== 0 /* ColumnType.Foreground */ || c.isVisible);
    }
    /**
    ...
    getBackgroundColumns() {
        return this.viewColumns.filter((c) => c.columnType === 1 /* ColumnType.Background */);
    }
    /**
    ...
    setWidthForHiddenColumns(visibleColumns) {
        ...
        let foreGroundColumn = this.viewColumns.find((column) => column.columnType === 0 /* ColumnType.Foreground */);
        if (foreGroundColumn) {
    ...
    async focus(viewColumn) {
        try {
            await this.busy;
            if (this.focusedColumn === viewColumn)
                return;
            ...
            else if (viewColumn.columnType === 0 /* ColumnType.Foreground */ && this.visibleBackgroundColumns.indexOf(viewColumn) < 0) {
    ...
    slideBackgroundColumns(nextVisibleViewColumn, oldOffset, newOffset) {
        return animations
            .add(this.domSlidingPart, transform("translateX" /* TransformEnum.TranslateX */, oldOffset, newOffset), {
            easing: ease.inOut,
        })
            .finally(() => {
    ...
    slideForegroundColumn(foregroundColumn, toForeground) {
        ...
        return animations
            .add(assertNotNull(foregroundColumn.domColumn, "foreground column has no domcolumn"), transform("translateX" /* TransformEnum.TranslateX */, oldOffset, newOffset), {
            easing: ease.in,
        })
            .finally(() => {
    ...
    updateOffsets() {
        ...
        for (let column of this.viewColumns) {
            if (column.columnType === 1 /* ColumnType.Background */ || column.isVisible) {
    ...
    isFirstBackgroundColumnFocused() {
        return this.viewColumns.filter((column) => column.columnType === 1 /* ColumnType.Background */).indexOf(this.focusedColumn) === 0;
    }
...
//# sourceMappingURL=ViewSlider.js.map