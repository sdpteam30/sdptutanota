...
import { AlarmServicePostTypeRef, createAlarmInfo, createAlarmNotification, createAlarmServicePost, createCalendarEventRef, createDateWrapper, createNotificationSessionKey, createRepeatRule, createUserAlarmInfo, PushIdentifierTypeRef, UserAlarmInfoTypeRef, } from "../../../entities/sys/TypeRefs.js";
import { assertNotNull, DAY_IN_MILLIS, downcast, flatMap, getFromMap, groupBy, groupByAndMap, groupByAndMapUniquely, isNotNull, neverNull, ofClass, promiseMap, stringToUtf8Uint8Array, } from "@tutao/tutanota-utils";
...
import { addDaysForEventInstance, addDaysForRecurringEvent, generateCalendarInstancesInRange, isClientOnlyCalendar, } from "../../../../calendar/date/CalendarUtils.js";
...
export class CalendarFacade {
    ...
    // visible for testing
    cachingEntityClient;
    constructor(userFacade, groupManagementFacade, 
    // We inject cache directly because we need to delete user from it for a hack
    entityRestCache, noncachingEntityClient, nativePushFacade, operationProgressTracker, instanceMapper, serviceExecutor, cryptoFacade, infoMessageHandler) {
    ...
    async saveCalendarEvents(eventsWrapper, onProgress) {
        ...
        for (const [listId, eventsWithAlarmsOfOneList] of eventsWithAlarmsByEventListId) {
            ...
            await this.cachingEntityClient
                .setupMultipleEntities(listId, eventsWithAlarmsOfOneList.map((e) => e.event))
                .catch(ofClass(SetupMultipleError, (e) => {
    ...
    async updateCalendarEvent(event, newAlarms, existingEvent) {
        ...
        if (existingEvent.uid == null)
            throw new Error("no uid set on the existing event");
    ...
    async getEventsByUid(uid, cacheMode = 0 /* CachingMode.Cached */) {
        ...
        for (const membership of memberships) {
            ...
            try {
                ...
                const indexEntry = await entityClient.load(CalendarEventUidIndexTypeRef, [
    ...
    getEntityClient(cacheMode) {
        if (cacheMode === 0 /* CachingMode.Cached */) {
 ...
 */
export function sortByRecurrenceId(arr) {
...
async function loadAlteredInstancesFromIndexEntry(entityClient, indexEntry) {
    if (indexEntry.alteredInstances.length === 0)
        return [];
...
//# sourceMappingURL=CalendarFacade.js.map