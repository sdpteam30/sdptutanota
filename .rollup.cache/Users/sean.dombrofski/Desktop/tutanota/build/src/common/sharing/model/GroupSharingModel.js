...
export class GroupSharingModel {
    ...
    static newAsync(info, eventController, entityClient, logins, mailFacade, shareFacade, groupManagementFacade, recipientsModel) {
        return entityClient
            ...
            .then((group) => Promise.all([entityClient.loadAll(SentGroupInvitationTypeRef, group.invitations), loadGroupMembers(group, entityClient)]).then(([sentGroupInvitations, memberInfos]) => new GroupSharingModel(info, group, memberInfos, sentGroupInvitations, eventController, entityClient, logins, mailFacade, shareFacade, groupManagementFacade, recipientsModel)));
    }
    dispose() {
    ...
    canRemoveGroupMember(member) {
        return ((hasCapabilityOnGroup(this.logins.getUserController().user, this.group, "2" /* ShareCapability.Invite */) || this.memberIsSelf(member)) &&
    ...
    canCancelInvitation(sentGroupInvitation) {
        return (hasCapabilityOnGroup(this.logins.getUserController().user, this.group, "2" /* ShareCapability.Invite */) ||
    ...
    async sendGroupInvitation(sharedGroupInfo, recipients, capability) {
        ...
        if (externalRecipients.length) {
            throw new UserError(lang.makeTranslation("featureTutanotaOnly_msg", lang.get("featureTutanotaOnly_msg") + " " + lang.get("invalidRecipients_msg") + "\n" + externalRecipients.join("\n")));
        }
        ...
        try {
            ...
            if (e instanceof RecipientsNotFoundError) {
                throw new UserError(lang.makeTranslation("tutanotaAddressDoesNotExist_msg", `${lang.get("tutanotaAddressDoesNotExist_msg")} ${lang.get("invalidRecipients_msg")}\n${e.message}`));
            }
            else {
                throw e;
            }
    ...
    entityEventsReceived(updates, eventOwnerGroupId) {
        return promiseMap(updates, (update) => {
            ...
            if (isUpdateForTypeRef(SentGroupInvitationTypeRef, update)) {
                if (update.operation === "0" /* OperationType.CREATE */ && isSameId(update.instanceListId, this.group.invitations)) {
                ...
                if (update.operation === "0" /* OperationType.CREATE */ && isSameId(update.instanceListId, this.group.members)) {
                    return this.entityClient
...
//# sourceMappingURL=GroupSharingModel.js.map