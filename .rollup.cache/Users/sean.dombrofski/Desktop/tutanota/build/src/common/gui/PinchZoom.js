...
export class PinchZoom {
    ...
    currentTouchStart = { x: 0, y: 0, startTime: 0 };
    // dragging below this threshold is not considered dragging, but noise
    static DRAG_THRESHOLD = 10;
    /// zooming
    pinchTouchIDs = new Set();
    lastPinchTouchPositions = { pointer1: { x: 0, y: 0 }, pointer2: { x: 0, y: 0 } };
    initialZoomablePosition = { x: 0, y: 0 };
    initialViewportPosition = { x: 0, y: 0 };
    pinchSessionTranslation = { x: 0, y: 0 };
    initialZoomableSize = { width: 0, height: 0 };
    zoomBoundaries = { min: 1, max: 3 };
    ...
    lastDoubleTapTouchStart = { x: 0, y: 0 };
    firstTapTime = 0;
    /**
    ...
    constructor(zoomable, viewport, initiallyZoomToViewportWidth, singleClickAction) {
        ...
        this.onTouchEndListener = this.zoomable.ontouchend = (e) => {
            ...
            if (e.touches.length === 0 && e.changedTouches.length === 1) {
                ...
                this.handleDoubleTap(e, eventTarget, (e, target) => singleClickAction(e, target), (e) => {
                    let scale = 1;
    ...
    rescale() {
        ...
        if (containerWidth >= this.zoomable.scrollWidth) {
            ...
            const newTransformOrigin = this.setCurrentSafePosition({ x: 0, y: 0 }, {
                x: 0,
                y: 0,
    ...
    pinchHandling(ev) {
        ...
        const scaleDifference = this.pointDistance({ x: ev.touches[0].clientX, y: ev.touches[0].clientY }, { x: ev.touches[1].clientX, y: ev.touches[1].clientY }) /
    ...
    dragHandling(ev) {
        if (this.currentScale > this.zoomBoundaries.min && this.lastDragTouchPosition) {
            if (Math.abs(ev.touches[0].clientX - this.currentTouchStart.x) >= PinchZoom.DRAG_THRESHOLD ||
    ...
    handleDoubleTap(event, target, singleClickAction, doubleClickAction) {
        ...
        if (now - this.firstTapTime < this.DOUBLE_TAP_TIME_MS &&
            ...
            setTimeout(() => {
                if (this.firstTapTime === now && // same touch, if a second tap was performed this condition is false
                    ...
                    if (now - this.currentTouchStart.startTime < this.LONG_PRESS_MIN_MS)
