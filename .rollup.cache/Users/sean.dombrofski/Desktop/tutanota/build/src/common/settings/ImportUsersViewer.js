...
export function checkAndImportUserData(userDetailsInputCsv, availableDomains) {
    let userData = csvToUserDetails(userDetailsInputCsv);
    if (!userData) {
        Dialog.message(lang.getTranslation("wrongUserCsvFormat_msg", {
...
async function showBookingDialog(userDetailsArray) {
    ...
    if (notAvailableUsers.length > 0) {
        await Dialog.message(lang.makeTranslation("addressesAlreadyInUse_msg", lang.get("addressesAlreadyInUse_msg") + " " + notAvailableUsers.map((u) => u.mailAddress).join(", ")));
    }
    await Dialog.message(lang.getTranslation("createdUsersCount_msg", {
        "{1}": nbrOfCreatedUsers,
    }));
}
/**
...
//# sourceMappingURL=ImportUsersViewer.js.map