...
import { ApprovalStatus, MAX_ATTACHMENT_SIZE } from "../api/common/TutanotaConstants.js";
import { assertNotNull, cleanMatch, deduplicate, defer, downcast, findAndRemove, getFromMap, LazyLoaded, neverNull, noOp, ofClass, promiseMap, remove, typedValues, } from "@tutao/tutanota-utils";
...
import { cloneInlineImages, revokeInlineImages } from "./inlineImagesUtils.js";
...
import { AccessBlockedError, LockedError, NotAuthorizedError, NotFoundError, PayloadTooLargeError, PreconditionFailedError, TooManyRequestsError, } from "../api/common/error/RestError.js";
...
export class SendMailModel {
    ...
    needNewDraft;
    initialized = null;
    onMailChanged = stream(null);
    onRecipientDeleted = stream(null);
    onBeforeSend = noOp;
    ...
    containsExternalRecipients() {
        return this.allRecipients().some((r) => r.type === "external" /* RecipientType.EXTERNAL */);
    }
    getExternalRecipients() {
        return this.allRecipients().filter((r) => r.type === "external" /* RecipientType.EXTERNAL */);
    }
    /**
    ...
    async doSaveDraft(saveAttachments, mailMethod) {
        ...
        try {
            ...
            const newAttachments = await promiseMap(attachmentIds, (fileId) => this.entity.load(FileTypeRef, fileId), {
                concurrency: 5,
            });
    ...
    async updateContacts(resolvedRecipients) {
        for (const { address, contact, type } of resolvedRecipients) {
            ...
            const isExternalAndConfidential = type === "external" /* RecipientType.EXTERNAL */ && this.isConfidential();
    ...
    async handleEntityEvent(update) {
        ...
        else if (isUpdateForTypeRef(MailboxPropertiesTypeRef, update) && operation === "1" /* OperationType.UPDATE */) {
...
//# sourceMappingURL=SendMailModel.js.map