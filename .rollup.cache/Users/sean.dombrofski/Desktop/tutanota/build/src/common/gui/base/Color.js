...
export const VALID_HEX_CODE_FORMAT = new RegExp("^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$");
 ...
 */
export function isValidColorCode(colorCode) {
    return VALID_HEX_CODE_FORMAT.test(colorCode);
}
export function isValidCSSHexColor(colorCode) {
    return isValidColorCode(colorCode) && CSS.supports("color", colorCode);
}
export function getColorLuminance(c) {
    const { r, g, b } = hexToRgb(c);
    // Counting the perceptive luminance
    // human eye favors green color...
    return (0.299 * r + 0.587 * g + 0.114 * b) / 255;
}
export function isMonochrome(c) {
    const { r, g, b } = hexToRgb(c);
    return r == g && g == b;
}
...
export function normalizeHueAngle(hue) {
...
export function hexToRGBAString(color, alpha) {
 ...
 */
export function expandHexTriplet(triplet) {
