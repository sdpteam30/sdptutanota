...
import { AccessBlockedError, AccessDeactivatedError, ConnectionError, handleRestError, NotAuthorizedError, ServiceUnavailableError, SessionExpiredError, } from "../common/error/RestError";
import { createWebsocketLeaderStatus, EntityEventBatchTypeRef, WebsocketCounterDataTypeRef, WebsocketEntityDataTypeRef, WebsocketLeaderStatusTypeRef, } from "../entities/sys/TypeRefs.js";
...
import { GroupType, SECOND_MS } from "../common/TutanotaConstants";
...
import { PhishingMarkerWebsocketDataTypeRef } from "../entities/tutanota/TypeRefs";
assertWorkerOrNode();
...
export class EventBusClient {
    ...
    state;
    socket;
    immediateReconnect = false; // if true tries to reconnect immediately after the websocket is closed
    /**
    ...
    connect(connectMode) {
        ...
        this.serviceUnavailableRetry = null;
        this.listener.onWebsocketStateChanged(0 /* WsConnectionState.connecting */);
    ...
    async tryReconnect(closeIfOpen, enableAutomaticState, delay = null) {
    ...
    onOpen(connectMode) {
        ...
        this.listener.onWebsocketStateChanged(1 /* WsConnectionState.connected */);
        return p;
    }
    onError(error) {
    ...
    async onMessage(message) {
        ...
        switch (type) {
            case "entityUpdate" /* MessageType.EntityUpdate */: {
            ...
            case "unreadCounterUpdate" /* MessageType.UnreadCounterUpdate */: {
            ...
            case "phishingMarkers" /* MessageType.PhishingMarkers */: {
            ...
            case "leaderStatus" /* MessageType.LeaderStatus */: {
    ...
    onClose(event) {
        ...
        this.userFacade.setLeaderStatus(createWebsocketLeaderStatus({
            leaderStatus: false,
        }));
    ...
    async initEntityEvents(connectMode) {
        ...
        this.eventQueue.pause();
        const existingConnection = connectMode == 1 /* ConnectMode.Reconnect */ && this.lastEntityEventIds.size > 0;
        ...
        return p
            ...
            this.close("reconnect" /* CloseEventBusOption.Reconnect */);
        }))
    ...
    async terminate() {
        this.state = "terminated" /* EventBusState.Terminated */;
        this.reset();
        this.listener.onWebsocketStateChanged(2 /* WsConnectionState.terminated */);
    }
    /**
    ...
    reconnect(closeIfOpen, enableAutomaticState) {
        ...
        if (closeIfOpen && this.socket && this.socket.readyState === WebSocket.OPEN) {
            ...
            this.connectTimer = setTimeout(() => this.connect(1 /* ConnectMode.Reconnect */), 100);
        }
    }
    ...
    async processEventBatch(batch) {
        try {
            ...
            if (!this.isTerminated())
                await this.listener.onEntityEventsReceived(filteredEvents, batch.batchId, batch.groupId);
        }
        catch (e) {
...
//# sourceMappingURL=EventBusClient.js.map