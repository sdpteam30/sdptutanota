...
export async function exportUserCsv(entityClient, logins, fileController, counterFacade) {
    const data = await loadUserExportData(entityClient, logins, counterFacade);
    const csv = renderCsv(["name", "mail address", "date created", "date deleted", "storage used (in bytes)", "aliases"], data.map((user) => [
        user.name,
        user.mailAddress,
 ...
 */
export async function loadUserExportData(entityClient, logins, counterFacade) {
...
//# sourceMappingURL=UserDataExporter.js.map