...
assertMainOrNode();
export function show(mailAddress, resetAction) {
    const selectedAction = stream(resetAction ?? null);
        ...
        okAction: async () => {
            ...
            if (!isMailAddress(cleanMailAddress, true)) {
                ...
                if (errorMessageId) {
                    ...
                    showProgressDialog("pleaseWait_msg", locator.loginFacade.recoverLogin(cleanMailAddress, cleanRecoverCodeValue, passwordModel.getNewPassword(), client.getIdentifier()))
                        .then(async () => {
                        recoverDialog.close();
...
//# sourceMappingURL=RecoverLoginDialog.js.map