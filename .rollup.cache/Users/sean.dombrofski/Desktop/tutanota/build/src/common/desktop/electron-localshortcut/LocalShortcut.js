...
export const ANY_WINDOW = Symbol("any-window");
 ...
 */
export function disableAll(win) {
 ...
 */
export function register(win, accelerator, callback) {
    ...
    const acceleratorString = accelerator;
    ...
    let shortcutsOfWindow = windowsWithShortcuts.get(wc);
 ...
 */
export function unregister(win, accelerator) {
 ...
 */
export function isRegistered(win, accelerator) {
    _checkAccelerator(accelerator);
    const wc = win.webContents;
    const shortcutsOfWindow = windowsWithShortcuts.get(wc);
    const eventStamp = toKeyEvent(accelerator);
...
//# sourceMappingURL=LocalShortcut.js.map