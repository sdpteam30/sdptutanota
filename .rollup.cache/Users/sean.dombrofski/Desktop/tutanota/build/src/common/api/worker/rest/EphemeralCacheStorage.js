...
import { assertNotNull, clone, getFromMap, remove } from "@tutao/tutanota-utils";
...
export class EphemeralCacheStorage {
    ...
    async get(typeRef, listId, elementId) {
        ...
        switch (typeModel.type) {
            case TypeId.Element:
                return clone(this.entities.get(path)?.get(elementId) ?? null);
            case TypeId.ListElement:
                return clone(this.lists.get(path)?.get(assertNotNull(listId))?.elements.get(elementId) ?? null);
            case TypeId.BlobElement:
                return clone(this.blobEntities.get(path)?.get(assertNotNull(listId))?.elements.get(elementId) ?? null);
            default:
    ...
    async provideFromRange(typeRef, listId, startElementId, count, reverse) {
        ...
        for (let a = 0; a < ids.length; a++) {
            result.push(clone(listCache.elements.get(ids[a])));
        }
        return result;
    }
    async provideMultiple(typeRef, listId, elementIds) {
        ...
        for (let a = 0; a < elementIds.length; a++) {
            result.push(clone(listCache.elements.get(elementIds[a])));
        }
        return result;
    }
...
//# sourceMappingURL=EphemeralCacheStorage.js.map