    ...
    async search(value) {
        ...
        if (this.loading != null) {
            ...
            if (query === this.currentQuery) {
                this.searchResults = [
                    ...newContactListSuggestions.map((value) => ({
                        type: "contactlist",
                        value,
                    })),
                    ...newContactSuggestions.map((value) => ({
                        type: "recipient",
                        value,
                    })),
    ...
    async findContacts(query) {
        ...
        for (const contact of contacts) {
            ...
            const filter = name.toLowerCase().indexOf(query) !== -1
                ? (address) => isMailAddress(address.trim(), false)
                : (address) => isMailAddress(address.trim(), false) && address.toLowerCase().indexOf(query) !== -1;
...
//# sourceMappingURL=RecipientsSearchModel.js.map