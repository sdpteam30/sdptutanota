...
import { asPaymentInterval, formatPrice, getPriceItem } from "./PriceUtils.js";
 ...
 */
export async function showBuyDialog(params) {
    ...
    if (priceChangeModel) {
        return showDialog(priceChangeModel.getActionLabel(), () => m(ConfirmSubscriptionView, { priceChangeModel, count: params.count, freeAmount: params.freeAmount, bookingText: params.bookingText }));
    }
    else {
        return false;
    }
...
class PriceChangeModel {
    ...
    constructor(price, featureType) {
        ...
        if (this.featureType === BookingItemFeatureType.LegacyUsers) {
            this.additionalFeatures = new Set([BookingItemFeatureType.Whitelabel, BookingItemFeatureType.Sharing, BookingItemFeatureType.Business].filter((f) => this.getFuturePrice(f) > 0));
        }
        else {
    ...
    isYearly() {
        ...
        return asPaymentInterval(period.paymentInterval) === 12 /* PaymentInterval.Yearly */;
    }
    taxIncluded() {
...
//# sourceMappingURL=BuyDialog.js.map