...
export class ServiceWorker {
    ...
    _urlsToCache;
    _isBuggyChrome;
    constructor(urlsToCache, caches, cacheName, selfLocation, applicationPaths, isTutanotaDomain) {
    ...
    precache() {
        return this._caches.open(this._cacheName).then((cache) => this._addAllToCache(cache, this._urlsToCache)
    ...
    async shouldTakeOverImmediately(cacheStorage) {
...
const init = (sw) => {
    ...
    scope.addEventListener("install", (evt) => {
        ...
        evt.waitUntil(sw.precache().then(async () => {
