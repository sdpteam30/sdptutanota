...
import { countryUsesGerman, getInvoiceItemTypeName } from "./InvoiceUtils.js";
const DE_POSTAL_CODE_REGEX = new RegExp(/\d{5}/);
const CITY_NAME_REGEX = new RegExp(/\d{5}/);
const PaymentMethodTypeCodes = Object.freeze({
    ["0" /* PaymentMethod.INVOICE */]: "31",
    ["1" /* PaymentMethod.CREDIT_CARD */]: "97",
    ["2" /* PaymentMethod.SEPA_UNUSED */]: "59",
    ["3" /* PaymentMethod.PAYPAL */]: "68",
    ["4" /* PaymentMethod.ACCOUNT_BALANCE */]: "97",
});
const VatTypeCategoryCodes = Object.freeze({
    ["0" /* VatType.NO_VAT */]: "E",
    ["1" /* VatType.ADD_VAT */]: "S",
    ["2" /* VatType.VAT_INCLUDED_SHOWN */]: "S",
    ["3" /* VatType.VAT_INCLUDED_HIDDEN */]: "S",
    ["4" /* VatType.NO_VAT_CHARGE_TUTAO */]: "AE",
});
/**
...
export class XRechnungInvoiceGenerator {
    ...
    generate() {
        let stringTemplate = `<?xml version="1.0" encoding="UTF-8"?>\n` +
            (this.invoice.invoiceType === "0" /* InvoiceType.INVOICE */ ? XRechnungUBLTemplate.RootInvoice : XRechnungUBLTemplate.RootCreditNote);
        stringTemplate = stringTemplate
            ...
            .replace("{paymentMeansCode}", PaymentMethodTypeCodes[this.invoice.paymentMethod])
     ...
     */
    resolveInvoiceType() {
        if (this.invoice.invoiceType === "0" /* InvoiceType.INVOICE */) {
    ...
    resolvePaymentNote() {
        ...
        if (this.invoice.invoiceType === "0" /* InvoiceType.INVOICE */) {
            switch (this.invoice.paymentMethod) {
                case "0" /* PaymentMethod.INVOICE */:
                    paymentNote += `${InvoiceTexts[this.languageCode].paymentInvoiceDue1} ${InvoiceTexts[this.languageCode].paymentInvoiceDue2} ${InvoiceTexts[this.languageCode].paymentInvoiceHolder} ${InvoiceTexts[this.languageCode].paymentInvoiceBank} ${InvoiceTexts[this.languageCode].paymentInvoiceIBAN} ${InvoiceTexts[this.languageCode].paymentInvoiceBIC} ${InvoiceTexts[this.languageCode].paymentInvoiceProvideNumber1} ${this.invoiceNumber} ${InvoiceTexts[this.languageCode].paymentInvoiceProvideNumber2}`;
                    break;
                ...
                case "3" /* PaymentMethod.PAYPAL */:
                    ...
                    break;
                case "4" /* PaymentMethod.ACCOUNT_BALANCE */:
    ...
    resolveAllowanceCharge() {
        return XRechnungUBLTemplate.AllowanceCharge.replace("{totalDiscount}", this.calculateTotalDiscount().toFixed(2))
            .replace("{vatType}", VatTypeCategoryCodes[this.invoice.vatType])
     ...
     */
    resolveTotalTax() {
        return XRechnungUBLTemplate.TaxTotal.replace("{vatType}", VatTypeCategoryCodes[this.invoice.vatType])
            .replace("{vatPercent}", this.invoice.vatRate)
            .replace("{slotTaxExemptionReason}", this.resolveTaxExemptionReason())
            .replace("{taxableAmount}", this.getVatExcludedPrice(this.invoice.subTotal))
            .replaceAll("{vatAmount}", this.invoice.vat);
    ...
    resolveTaxExemptionReason() {
        ...
        if (this.invoice.vatType === "0" /* VatType.NO_VAT */ || this.invoice.vatType === "4" /* VatType.NO_VAT_CHARGE_TUTAO */) {
    ...
    resolveDocumentsTotal() {
        return XRechnungUBLTemplate.DocumentTotals.replace("{sumOfInvoiceLines}", this.getVatExcludedPrice((parseFloat(this.invoice.subTotal) + this.calculateTotalDiscount()).toFixed(2)))
    ...
    resolveInvoiceLine(invoiceItem) {
        ...
        return XRechnungUBLTemplate.InvoiceLine.replace("{invoiceLineId}", this.itemIndex.toString())
            ...
            .replace("{invoiceLineItemVatType}", VatTypeCategoryCodes[this.invoice.vatType])
    ...
    getVatExcludedPrice(priceValue) {
        switch (this.invoice.vatType) {
            case "2" /* VatType.VAT_INCLUDED_SHOWN */:
            case "3" /* VatType.VAT_INCLUDED_HIDDEN */: {
 ...
 * @param addressLine
 */
export function extractPostalCode(addressLine) {
 ...
 * @param addressLine
 */
export function extractCityName(addressLine) {
...
//# sourceMappingURL=XRechnungInvoiceGenerator.js.map