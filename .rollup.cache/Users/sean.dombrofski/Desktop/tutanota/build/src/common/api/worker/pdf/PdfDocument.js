import { boldFontWidths, PdfStreamEncoding, regularFontWidths } from "./PdfConstants.js";
import { Deflater } from "./Deflater.js";
import { stringToUtf8Uint8Array } from "@tutao/tutanota-utils";
export var PDF_FONTS;
(function (PDF_FONTS) {
    PDF_FONTS[PDF_FONTS["REGULAR"] = 1] = "REGULAR";
    PDF_FONTS[PDF_FONTS["BOLD"] = 2] = "BOLD";
    PDF_FONTS[PDF_FONTS["INVISIBLE_CID"] = 3] = "INVISIBLE_CID";
})(PDF_FONTS || (PDF_FONTS = {}));
export var PDF_IMAGES;
(function (PDF_IMAGES) {
    PDF_IMAGES[PDF_IMAGES["TUTA_LOGO"] = 1] = "TUTA_LOGO";
    PDF_IMAGES[PDF_IMAGES["ADDRESS"] = 2] = "ADDRESS";
})(PDF_IMAGES || (PDF_IMAGES = {}));
export var TEXT_RENDERING_MODE;
(function (TEXT_RENDERING_MODE) {
    TEXT_RENDERING_MODE[TEXT_RENDERING_MODE["NORMAL"] = 0] = "NORMAL";
    TEXT_RENDERING_MODE[TEXT_RENDERING_MODE["INVISIBLE"] = 3] = "INVISIBLE";
...
export class PdfDocument {
    ...
    addText(text, position = ORIGIN_POSITION, byteLength = 1) {
        ...
        if (position !== ORIGIN_POSITION) {
            this.textStream += `1 0 0 -1 ${mmToPSPoint(position[0])} ${mmToPSPoint(position[1]) + this.currentFontSize} Tm <${toUnicodePoint(text, byteLength).join("")}> Tj `;
        }
        else {
    ...
    addTableRow(position, columnInfo, rowItems) {
        if (rowItems.length !== columnInfo.length)
            console.error("Amount of items in table row not equal to amount of columns!");
 ...
 */
export function toUnicodePoint(input, byteLength = 1) {
    if (byteLength === 1) {
        const out = [];
 ...
 * @param codePoint
 */
export function isCodePointOneByteLength(codePoint) {
 ...
 * @param stringParts
 */
export function areStringPartsOneByteLength(stringParts) {
 ...
 */
export function getWordLengthInPoints(codePoints, font, fontSize) {
