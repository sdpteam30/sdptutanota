...
import { convertHeaders, customFetch, toGlobalResponse } from "./NetAgent";
 ...
 */
export function handleProtocols(session, assetDir) {
    doHandleProtocols(session, assetDir, customFetch, path, fs);
}
/**
 *  exported for testing
 */
export function doHandleProtocols(session, assetDir, fetchImpl, pathModule, fsModule) {
    if (!interceptProtocol("http", session, fetchImpl))
        throw new Error("could not intercept http protocol");
    if (!interceptProtocol("https", session, fetchImpl))
        throw new Error("could not intercept https protocol");
    if (!handleAssetProtocol(session, assetDir, pathModule, fsModule))
        throw new Error("could not register asset protocol");
}
/**
 ...
 */
function interceptProtocol(protocol, session, fetchImpl) {
    if (session.protocol.isProtocolHandled(protocol))
        return true;
    session.protocol.handle(protocol, async (request) => {
        ...
        if (!url.startsWith(protocol)) {
            ...
            try {
                const options = { method, keepalive: true };
...
function handleAssetProtocol(session, assetDir, pathModule, fsModule) {
    if (session.protocol.isProtocolHandled(ASSET_PROTOCOL))
        return true;
    session.protocol.handle(ASSET_PROTOCOL, async (request) => {
        const fail = (msg) => {
            log.debug(TAG, msg);
 ...
 */
export async function fileFetch(filePath, fsModule) {
...
//# sourceMappingURL=ProtocolProxy.js.map