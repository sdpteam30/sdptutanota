...
import { lazyMemoized, noOp } from "@tutao/tutanota-utils";
...
export async function initLocator(worker, browserData) {
    ...
    if (isOfflineStorageAvailable()) {
        ...
        offlineStorageProvider = async () => {
            return new OfflineStorage(locator.sqlCipherFacade, new InterWindowEventFacadeSendDispatcher(worker), dateProvider, new OfflineStorageMigrator(OFFLINE_STORAGE_MIGRATIONS, modelInfos), new CalendarOfflineCleaner());
        };
    }
    else {
    ...
    locator.cacheManagement = lazyMemoized(async () => {
        ...
        return new CacheManagementFacade(locator.user, locator.cachingEntityClient, locator.cache);
    });
    ...
    const asymmetricCrypto = new AsymmetricCryptoFacade(locator.rsa, locator.pqFacade, locator.keyLoader, cryptoWrapper, locator.serviceExecutor, locator.publicKeyProvider);
    locator.crypto = new CryptoFacade(locator.user, locator.cachingEntityClient, locator.restClient, locator.serviceExecutor, locator.instanceMapper, new OwnerEncSessionKeysUpdateQueue(locator.user, locator.serviceExecutor), locator.cache, locator.keyLoader, asymmetricCrypto, locator.publicKeyProvider, lazyMemoized(() => locator.keyRotation));
    ...
    locator.groupManagement = lazyMemoized(async () => {
        ...
        return new GroupManagementFacade(locator.user, await locator.counters(), locator.cachingEntityClient, locator.serviceExecutor, locator.pqFacade, locator.keyLoader, await locator.cacheManagement(), asymmetricCrypto, cryptoWrapper, keyAuthenticationFacade);
    });
    locator.keyRotation = new KeyRotationFacade(locator.cachingEntityClient, locator.keyLoader, locator.pqFacade, locator.serviceExecutor, cryptoWrapper, locator.recoverCode, locator.user, locator.crypto, locator.share, locator.groupManagement, asymmetricCrypto, keyAuthenticationFacade, locator.publicKeyProvider);
    const loginListener = {
    ...
    locator.calendar = lazyMemoized(async () => {
        ...
        return new CalendarFacade(locator.user, await locator.groupManagement(), locator.cache, nonCachingEntityClient, // without cache
    ...
    locator.eventBusClient = new EventBusClient(eventBusCoordinator, locator.cache, locator.user, locator.cachingEntityClient, locator.instanceMapper, (path) => new WebSocket(getWebsocketBaseUrl(domainConfig) + path), new SleepDetector(scheduler, dateProvider), mainInterface.progressTracker);
...
//# sourceMappingURL=CalendarWorkerLocator.js.map