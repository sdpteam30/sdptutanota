...
export class CalendarAgendaView {
    ...
    renderDateSelector(attrs, isDesktopLayout, selectedDate) {
        ...
        return isDesktopLayout
            ? null
            : m("", m(".header-bg.pb-s.overflow-hidden", {
                style: {
                    "margin-left": px(size.calendar_hour_width_mobile),
                },
            }, m(DaySelector, {
                selectedDate: selectedDate,
                onDateSelected: (selectedDate) => attrs.onDateSelected(selectedDate),
                wide: true,
    ...
    renderMobileEventList(day, attrs) {
        ...
        if (events.length === 0) {
            ...
            return m(".pt-s.flex.mb-s.col.overflow-y-scroll.height-100p", {
    ...
    renderAgenda(attrs, isDesktopLayout) {
        if (!isDesktopLayout)
            return this.renderMobileAgendaView(attrs);
        return m(".flex.flex-grow.height-100p", [
            m(".flex-grow.rel.overflow-y-scroll", {
                style: {
    ...
    getBirthdayEventModel(eventPreviewModel) {
        if (isBirthdayEvent(eventPreviewModel.event?.uid)) {
    ...
    renderEventPreview(attrs) {
        ...
        if (model) {
            return m(".flex.col", m(ContactCardViewer, {
        ...
        return m(EventDetailsView, {
            eventPreviewModel: eventPreviewModel,
        });
    }
    ...
    renderEventsForDay(events, zone, day, attrs) {
        ...
        for (const [eventIndex, event] of events.entries()) {
            ...
            eventsNodes.push(m(CalendarAgendaItemView, {
        ...
        if (attrs.scrollPosition === this.lastScrollPosition)
            attrs.onScrollPositionChange(newScrollPosition - (agendaItemHeight + agendaGap));
 ...
 */
export function earliestEventToShowTimeIndicator(events, date) {
...
//# sourceMappingURL=CalendarAgendaView.js.map