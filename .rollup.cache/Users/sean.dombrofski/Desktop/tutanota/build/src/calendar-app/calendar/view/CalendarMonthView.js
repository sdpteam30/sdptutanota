...
import { getAllDayDateForTimezone, getDiffIn24hIntervals, getEventEnd, getFirstDayOfMonth, getStartOfNextDayWithZone, getStartOfTheWeekOffset, getTimeZone, getWeekNumber, } from "../../../common/calendar/date/CalendarUtils";
...
import { CALENDAR_EVENT_HEIGHT, changePeriodOnWheel, getCalendarMonth, getDateFromMousePos, getEventColor, layOutEvents, SELECTED_DATE_INDICATOR_THICKNESS, TEMPORARY_EVENT_OPACITY, } from "../gui/CalendarGuiUtils.js";
...
export class CalendarMonthView {
    ...
    view({ attrs }) {
        ...
        return m(".fill-absolute.flex.col", {
            ...
            m(".flex.col.rel.flex-grow.overflow-hidden", {
                class: (!styles.isUsingBottomNavigation() || (isIOSApp() && client.isCalendarApp()) ? "content-bg" : "") +
    ...
    renderCalendar(attrs, month, currentlyVisibleMonth, zone) {
        ...
        return m(".fill-absolute.flex.col.flex-grow", {
            oncreate: (vnode) => {
                if (isVisible) {
    ...
    renderDay(attrs, day, weekDayNumber, firstWeek) {
        return m(".calendar-day.calendar-column-border.flex-grow.rel.overflow-hidden.fill-absolute.cursor-pointer", {
            ...
            weekDayNumber === 0 && attrs.startOfTheWeek === "0" /* WeekStart.MONDAY */
                ? m(".calendar-month-week-number.abs.z3", {
                    onclick: (e) => {
                        e.stopPropagation();
    ...
    renderDayHeader({ date, day, isPaddingDay }, onDateSelected) {
        ...
        return m(".rel.click.flex.items-center.justify-center.rel.ml-hpad_small", {
    ...
    renderEvent(event, position, eventStart, firstDayOfWeek, firstDayOfNextWeek, eventEnd, attrs, isDisabled) {
        ...
        return m(".abs.overflow-hidden", {
            ...
            showTime: styles.isDesktopLayout() && !isAllDayEvent(event) ? "startTime" /* EventTextTimeOption.START_TIME */ : null,
