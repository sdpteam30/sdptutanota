...
import { createCalendarEvent, createEncryptedMailAddress, } from "../../../../common/api/entities/tutanota/TypeRefs.js";
import { areRepeatRulesEqual, DefaultDateProvider, findFirstPrivateCalendar, getTimeZone, incrementSequence, parseAlarmInterval, } from "../../../../common/calendar/date/CalendarUtils.js";
import { arrayEqualsWithPredicate, assertNonNull, assertNotNull, identity } from "@tutao/tutanota-utils";
...
import { getStrippedClone } from "../../../../common/api/common/utils/EntityUtils.js";
import { CalendarNotificationModel } from "./CalendarNotificationModel.js";
import { CalendarEventApplyStrategies } from "./CalendarEventModelStrategy.js";
 ...
 */
export async function makeCalendarEventModel(operation, initialValues, recipientsModel, calendarModel, logins, mailboxDetail, mailboxProperties, sendMailModelFactory, notificationSender, entityClient, responseTo, zone = getTimeZone(), showProgress = identity, uiUpdateCallback = m.redraw) {
    ...
    if (operation === 4 /* CalendarOperation.DeleteAll */ || operation === 3 /* CalendarOperation.EditAll */) {
    ...
    const makeEditModels = (initializationEvent) => ({
        ...
        whoModel: new CalendarEventWhoModel(initializationEvent, eventType, operation, calendars, selectedCalendar, logins.getUserController(), operation === 0 /* CalendarOperation.Create */, ownMailAddresses, recipientsModel, responseTo, getPasswordStrength, sendMailModelFactory, uiUpdateCallback),
...
async function selectStrategy(makeEditModels, applyStrategies, operation, resolveProgenitor, existingInstanceIdentity, cleanInitialValues) {
    ...
    else if (operation === 2 /* CalendarOperation.DeleteThis */) {
        if (cleanInitialValues.recurrenceId == null) {
            const progenitor = await resolveProgenitor();
            if (progenitor == null) {
                return null;
    ...
    else if (operation === 3 /* CalendarOperation.EditAll */) {
        const progenitor = await resolveProgenitor();
        if (progenitor == null) {
            return null;
        }
    ...
    else if (operation === 4 /* CalendarOperation.DeleteAll */) {
        editModels = makeEditModels(cleanInitialValues);
...
export function getNonOrganizerAttendees({ organizer, attendees, }) {
    if (attendees == null)
        return [];
    if (organizer == null)
        return attendees;
...
export class CalendarEventModel {
    ...
    constructor(strategy, eventType, operation, 
    ...
    userController, distributor, entityClient, calendars) {
    ...
    async apply() {
        ...
        try {
            ...
            if (e instanceof PayloadTooLargeError) {
                ...
                return 2 /* EventSaveResult.NotFound */;
            }
            else {
                throw e;
            }
    ...
    isFullyWritable() {
        return this.eventType === "own" /* EventType.OWN */ || this.eventType === "shared_rw" /* EventType.SHARED_RW */;
    }
     ...
     * */
    canEditSeries() {
        return this.operation !== 1 /* CalendarOperation.EditThis */ && (this.eventType === "own" /* EventType.OWN */ || this.eventType === "shared_rw" /* EventType.SHARED_RW */);
    }
    canChangeCalendar() {
        return (this.operation !== 1 /* CalendarOperation.EditThis */ &&
            (this.eventType === "own" /* EventType.OWN */ || this.eventType === "shared_rw" /* EventType.SHARED_RW */ || this.eventType === "invite" /* EventType.INVITE */));
    }
    isAskingForUpdatesNeeded() {
        return (this.eventType === "own" /* EventType.OWN */ &&
    ...
    getReadonlyReason() {
        ...
        if (isFullyWritable && canEditSeries && canModifyGuests)
            return 4 /* ReadonlyReason.NONE */;
        if (!isFullyWritable && !canEditSeries && !canModifyGuests)
            return 2 /* ReadonlyReason.NOT_ORGANIZER */;
        // fully writable and !canModifyGuests happens on shared calendars
        if (!canModifyGuests) {
            if (canEditSeries) {
                return 0 /* ReadonlyReason.SHARED */;
            }
            else {
                return 1 /* ReadonlyReason.SINGLE_INSTANCE */;
            }
        }
        return 3 /* ReadonlyReason.UNKNOWN */;
    }
}
/**
 *
 ...
 */
export function eventHasChanged(now, previous) {
    if (previous == null)
        return true;
    ...
    return (now.startTime.getTime() !== previous?.startTime?.getTime() ||
        ...
        !arrayEqualsWithPredicate(now.attendees, previous?.attendees ?? [], (a1, a2) => a1.status === a2.status && cleanMailAddress(a1.address.address) === cleanMailAddress(a2.address.address)) || // we ignore the names
        (now.organizer !== previous.organizer && now.organizer?.address !== previous.organizer?.address)); // we ignore the names
}
/**
 ...
 * @param models
 */
export function assembleCalendarEventEditResult(models) {
 ...
 */
export function assembleEditResultAndAssignFromExisting(existingEvent, editModels, operation) {
    ...
    const newEvent = assignEventIdentity(assembleResult.eventValues, {
        ...
        recurrenceId: operation === 1 /* CalendarOperation.EditThis */ && recurrenceId == null ? existingEvent.startTime : recurrenceId,
    });
 ...
 */
export function assignEventIdentity(values, identity) {
...
function getPreselectedCalendar(calendars, event) {
    ...
    if (ownerGroup == null || !calendars.has(ownerGroup)) {
        ...
        if (!calendar)
            throw new Error("Can't find a private calendar");
        return calendar;
    }
    else {
...
//# sourceMappingURL=CalendarEventModel.js.map