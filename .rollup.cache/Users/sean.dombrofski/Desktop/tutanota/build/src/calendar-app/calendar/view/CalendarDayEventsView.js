...
import { eventEndsAfterDay, eventStartsBefore, getTimeTextFormatForLongEvent, getTimeZone, hasAlarmsForTheUser, isClientOnlyCalendar, } from "../../../common/calendar/date/CalendarUtils";
import { CalendarEventBubble } from "./CalendarEventBubble";
import { Time } from "../../../common/calendar/date/Time.js";
import { getPosAndBoundsFromMouseEvent } from "../../../common/gui/base/GuiUtils";
import { expandEvent, formatEventTime, getEventColor, getDisplayEventTitle, getTimeFromMousePos, layOutEvents, TEMPORARY_EVENT_OPACITY, } from "../gui/CalendarGuiUtils.js";
...
export const calendarDayTimes = numberRange(0, 23).map((number) => new Time(number, 0));
const allHoursHeight = size.calendar_hour_height * calendarDayTimes.length;
export class CalendarDayEventsView {
    dayDom = null;
    view({ attrs }) {
        return m(".col.rel", {
            oncreate: (vnode) => {
    ...
    renderEvents(attrs, events) {
        return layOutEvents(events, getTimeZone(), (columns) => this.renderColumns(attrs, columns), 0 /* EventLayoutMode.TimeBasedColumn */);
    }
...
function getTimeFromClickInteraction(e, time) {
    ...
    if (mousePositionRelativeToRectHeight > rect.height / 2)
        return new Time(time.hour, time.minute + 30);
    return time;
}
//# sourceMappingURL=CalendarDayEventsView.js.map