import { assertEnumValue, RepeatPeriod, SECOND_MS } from "../../../common/api/common/TutanotaConstants";
...
export function makeInvitationCalendarFile(event, method, now, zone) {
...
export function serializeCalendar(versionNumber, events, now, zone) {
...
export function serializeEvent(event, alarms, now, timeZone) {
    ...
    return [
        ...
        .concat(event.recurrenceId != null
        ? isAllDay
...
export function serializeRepeatRule(repeatRule, isAllDayEvent, localTimeZone) {
    if (repeatRule) {
        let endType = "";
...
/** importer internals exported for testing */
export function serializeExcludedDates(excludedDates, timeZone) {
...
export function serializeTrigger(dbAlarmInterval) {
...
function serializeIcalText(value) {
    ...
    for (const rawEscape in iCalReplacements) {
        text = text.replaceAll(rawEscape, iCalReplacements[rawEscape]);
    }
    return text;
}
...
//# sourceMappingURL=CalendarExporter.js.map