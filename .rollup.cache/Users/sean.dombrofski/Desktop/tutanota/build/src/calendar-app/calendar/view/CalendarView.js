...
import { CalendarGroupRootTypeRef, ContactTypeRef, createDefaultAlarmInfo, createGroupSettings, } from "../../../common/api/entities/tutanota/TypeRefs.js";
import { DEFAULT_CLIENT_ONLY_CALENDAR_COLORS, defaultCalendarColor, GroupType, Keys, NewPaidPlans, reverse, } from "../../../common/api/common/TutanotaConstants";
import { locator } from "../../../common/api/main/CommonLocator";
import { CalendarType, findFirstPrivateCalendar, getCalendarType, getStartOfTheWeekOffset, getStartOfTheWeekOffsetForUser, getTimeZone, getWeekNumber, hasSourceUrl, isBirthdayEvent, isClientOnlyCalendar, parseAlarmInterval, } from "../../../common/calendar/date/CalendarUtils";
import { CalendarMonthView } from "./CalendarMonthView";
import { DateTime } from "luxon";
import { NotFoundError } from "../../../common/api/common/error/RestError";
import { CalendarAgendaView } from "./CalendarAgendaView";
import { handleUrlSubscription, showCreateEditCalendarDialog } from "../gui/EditCalendarDialog.js";
...
import { getExternalCalendarName, parseCalendarStringData } from "../../../common/calendar/import/ImportExportUtils.js";
...
var RenderType;
(function (RenderType) {
    RenderType[RenderType["Private"] = 0] = "Private";
    RenderType[RenderType["Shared"] = 1] = "Shared";
    RenderType[RenderType["External"] = 2] = "External";
    RenderType[RenderType["ClientOnly"] = 3] = "ClientOnly";
...
export class CalendarView extends BaseTopLevelView {
                ...
                content: [
                    ...
                    m(SidebarSection, {
                        ...
                        button: m(IconButton, {
                            ...
                            click: (isApp() || isDesktop()) && findFirstPrivateCalendar(attrs.calendarViewModel.calendarInfos)
                                ? createDropdown({
                                    lazyButtons: () => [
                                        {
                    ...
                    this.viewModel.calendarInvitations().length > 0
                        ...
                        }, this.viewModel.calendarInvitations().map((invitation) => m(GroupInvitationFolderRow, {
                            invitation,
                        })))
                        : null,
                ],
                    ...
                    case CalendarViewType.MONTH:
                        return m(BackgroundColumnLayout, {
                            ...
                            columnLayout: m(CalendarMonthView, {
                                ...
                                amPmFormat: locator.logins.getUserController().userSettingsGroupRoot.timeFormat === "1" /* TimeFormat.TWELVE_HOURS */,
                                ...
                                onEventKeyDown: (event, domEvent) => {
                                    if (isKeyPressed(domEvent.key, Keys.RETURN, Keys.SPACE) && !domEvent.repeat) {
                                        if (styles.isDesktopLayout()) {
                                            ...
                                            this.showCalendarEventPopupAtEvent(event, domEvent.target, this.htmlSanitizer);
                                        }
                                    }
        ...
        return [
            ...
            },
            {
                ...
                exec: () => {
                    ...
                    if (viewSize)
                        this.viewModel.scroll(-viewSize);
                },
    ...
    async createNewEventDialog(date = null) {
        ...
        const model = await locator.calendarEventModel(0 /* CalendarOperation.Create */, getEventWithDefaultTimes(dateToUse), mailboxDetails, mailboxProperties, null);
        if (model) {
            ...
            for (const renderType of renderTypes) {
                switch (renderType) {
                    case RenderType.ClientOnly:
                        conditions.push((calendarInfo) => isClientOnlyCalendar(calendarInfo.group._id));
                        break;
                    case RenderType.Private:
                        conditions.push((calendarInfo) => calendarInfo.userIsOwner && !calendarInfo.isExternal && !isClientOnlyCalendar(calendarInfo.group._id));
                        break;
                    case RenderType.Shared:
                        conditions.push((calendarInfo) => !calendarInfo.userIsOwner);
                        break;
                    case RenderType.External:
                        conditions.push((calendarInfo) => calendarInfo.userIsOwner && calendarInfo.isExternal);
                        break;
                }
            }
    ...
    renderCalendarItem(calendarInfo, shared, toggleHidden, isHidden) {
        ...
        const handleToggleCalendar = () => {
            if (!isClientOnlyCalendar(groupRootId) || this.viewModel.isNewPaidPlan)
        ...
        return m(".folder-row.flex-start.plr-button", [
            m(".flex.flex-grow.center-vertically.button-height", [
                ...
                m(".pl-m.b.flex-grow.text-ellipsis", {
                    style: {
                        width: 0,
                    },
                }, groupName),
            ]),
            hasSourceUrl(existingGroupSettings) && lastSyncEntry?.lastSyncStatus === "Failed" /* SyncStatus.Failed */
                ? m(Icon, {
                    title: lastSyncStr,
                    icon: "SyncProblem" /* Icons.SyncProblem */,
            ...
            click: createDropdown({
                lazyButtons: () => [
                    ...
                    this.allowCalendarImport(group, user, existingGroupSettings)
                        ...
                        : null,
                    !isApp() && group.type === GroupType.Calendar && hasCapabilityOnGroup(user, group, "0" /* ShareCapability.Read */) && !isClientOnly
                        ? {
                            label: "export_action",
                            ...
                            click: () => {
                                ...
                                if (alarmInfoList) {
                                    exportCalendar(getSharedGroupName(groupInfo, locator.logins.getUserController(), sharedCalendar), groupRoot, alarmInfoList.alarms, new Date(), getTimeZone());
                                }
                            },
                        }
                        : null,
    ...
    allowCalendarImport(group, user, groupSettings) {
        return (group.type === GroupType.Calendar &&
            hasCapabilityOnGroup(user, group, "1" /* ShareCapability.Write */) &&
    ...
    confirmDeleteCalendar(calendarInfo) {
        ...
        loadGroupMembers(calendarInfo.group, locator.entityClient).then((members) => {
            ...
            Dialog.confirm(lang.makeTranslation("confirm_msg", (otherMembers.length > 0
    ...
    buildRouteState(view, resetState, dateString) {
        ...
        if (m.route.get().includes(CalendarViewType.MONTH) ||
