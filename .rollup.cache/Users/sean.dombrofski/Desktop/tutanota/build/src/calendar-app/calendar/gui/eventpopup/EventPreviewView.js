...
import { CalendarAttendeeStatus, getAttendeeStatus, RepeatPeriod } from "../../../../common/api/common/TutanotaConstants.js";
...
export const ReplyButtons = pureComponent((participation) => {
    const colors = {
...
export class EventPreviewView {
    ...
    view(vnode) {
        ...
        return m(".flex.col.smaller.scroll.visible-scrollbar", [
            ...
            this.renderRow("Time" /* Icons.Time */, [formatEventDuration(event, getTimeZone(), false), this.renderRepeatRule(event.repeatRule, isAllDayEvent(event))]),
    ...
    renderLocation(location) {
        if (location == null || location.trim().length === 0)
            return null;
        return this.renderRow("Pin" /* Icons.Pin */, [
            m(".text-ellipsis.selectable", m(ExternalLink, {
    ...
    renderAttendeesSection(attendees, participation) {
        if (attendees.length === 0)
            return null;
        return this.renderRow("People" /* Icons.People */, [
            m(".flex-wrap", attendees.map((a) => this.renderAttendee(a, participation))),
        ], true);
    }
    /**
    ...
    renderAttendanceSection(event, attendees, participation) {
        if (attendees.length === 0 || participation == null || event._ownerGroup == null)
            return null;
        return m(".flex.pb-s", [this.renderSectionIndicator("Contacts" /* BootIcons.Contacts */), m(ReplyButtons, participation)]);
    }
    renderAttendee(attendee, participation) {
        ...
        const status = participation != null && cleanMailAddress(attendee.address.address) === participation.ownAttendee.address.address
    ...
    renderDescription(sanitizedDescription) {
        if (sanitizedDescription == null || sanitizedDescription.length === 0)
            return null;
        return this.renderRow("AlignLeft" /* Icons.AlignLeft */, [m.trust(sanitizedDescription)], true);
    }
}
/**
 ...
 * @returns {*}
 */
export function getLocationUrl(text) {
...
export function formatRepetitionFrequency(repeatRule) {
 ...
 */
export function formatRepetitionEnd(repeatRule, isAllDay) {
...
function prepareAttendees(attendees, organizer) {
    ...
    if (organizer != null && attendeesCopy.length > 0 && !findAttendeeInAddresses(attendeesCopy, [organizer.address])) {
        attendeesCopy.unshift(createCalendarEventAttendee({
...
//# sourceMappingURL=EventPreviewView.js.map