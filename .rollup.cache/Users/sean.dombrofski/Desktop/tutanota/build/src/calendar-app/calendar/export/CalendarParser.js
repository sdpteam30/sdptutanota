...
import { createCalendarEvent, createCalendarEventAttendee, createEncryptedMailAddress, } from "../../../common/api/entities/tutanota/TypeRefs.js";
import { createCalendarAdvancedRepeatRule } from "../../../common/api/entities/sys/TypeRefs.js";
import { createDateWrapper, createRepeatRule } from "../../../common/api/entities/sys/TypeRefs.js";
import { combineParsers, makeCharacterParser, makeEitherParser, makeNotCharacterParser, makeSeparatedByParser, makeZeroOrMoreParser, mapParser, maybeParse, numberParser, ParserError, StringIterator, } from "../../../common/misc/parsing/ParserCombinator";
import WindowsZones from "./WindowsZones";
import { isMailAddress } from "../../../common/misc/FormatValidator";
import { CalendarAttendeeStatus, CalendarMethod, RepeatPeriod, reverse } from "../../../common/api/common/TutanotaConstants";
...
function getProp(obj, tag, optional) {
    ...
    if (!optional && prop == null)
        throw new ParserError(`Missing prop ${tag}`);
    return prop;
}
function getPropStringValue(obj, tag, optional) {
    const prop = getProp(obj, tag, optional);
    if (!optional && typeof prop?.value !== "string")
        throw new ParserError(`value of ${tag} is not of type string, got ${JSON.stringify(prop)}`);
    return prop?.value;
}
...
const parsePropertyParameters = combineParsers(makeCharacterParser(";"), makeSeparatedByParser(/*separator*/ makeCharacterParser(";"), /*value*/ propertyParametersKeyValueParser));
 ...
 */
export const propertySequenceParser = combineParsers(parsePropertyName, maybeParse(parsePropertyParameters), makeCharacterParser(":"), anyStringUnescapeParser);
 ...
 * Parses multiple key=value pair on the right side of the semicolon (value side)
 */
export function parsePropertyKeyValue(data) {
    ...
    while (!iteration.done && iteration.value) {
        ...
        if (property.name === "BEGIN") {
            if (typeof property.value !== "string")
                throw new ParserError("BEGIN with array value");
...
export function parseICalendar(stringData) {
...
/** visible for testing */
export function triggerToAlarmInterval(eventStart, triggerValue) {
    // Absolute time
...
export function parseRrule(rawRruleValue, tzId) {
    let rruleValue;
    try {
    ...
    const endType = until != null ? "2" /* EndType.UntilDate */ : count != null ? "1" /* EndType.Count */ : "0" /* EndType.Never */;
...
export function parseAdvancedRule(rrule) {
...
export function parseExDates(excludedDatesProps) {
...
export function parseRecurrenceId(recurrenceIdProp, tzId) {
...
export const calendarAttendeeStatusToParstat = {
...
export function parseCalendarEvents(icalObject, zone) {
...
export function repeatPeriodToIcalFrequency(repeatPeriod) {
...
/** parse a time */
export function parseTimeIntoComponents(value) {
...
export function parseUntilRruleTime(value, zone) {
