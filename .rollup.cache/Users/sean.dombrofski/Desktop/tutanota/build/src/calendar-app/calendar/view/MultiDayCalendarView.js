...
import { combineDateWithTime, eventEndsAfterDay, eventStartsBefore, getDiffIn24hIntervals, getEventEnd, getEventStart, getRangeOfDays, getStartOfTheWeekOffset, getStartOfWeek, getTimeTextFormatForLongEvent, getTimeZone, } from "../../../common/calendar/date/CalendarUtils";
...
import { CALENDAR_EVENT_HEIGHT, daysHaveEvents, getEventColor, layOutEvents, TEMPORARY_EVENT_OPACITY } from "../gui/CalendarGuiUtils.js";
...
export class MultiDayCalendarView {
    longEventsDom = null;
    domElements = [];
    eventDragHandler;
    dateUnderMouse = null;
        ...
        return m(".flex.col.fill-absolute", [
            !isDesktopLayout
                ? [
                    ...
                    this.renderHeaderMobile(isDayView ? currentPageEvents : weekEvents, attrs.groupColors, attrs.onEventClicked, attrs.onEventKeyDown, attrs.temporaryEvents),
                ]
                : null,
            m(PageView, {
                previousPage: {
    ...
    renderDateSelector(attrs, isDayView) {
        return m("", [
            m(".header-bg.pb-s.overflow-hidden", {
                style: {
    ...
    renderDays(attrs, thisPeriod, mainPeriod, isDayView, isDesktopLayout) {
        ...
        return m(".fill-absolute.flex.col.overflow-hidden", {
            ...
            m(".flex.scroll-no-overlay.content-bg", {
                oncreate: (vnode) => {
                ...
                m(".flex.col", calendarDayTimes.map((time) => {
                    ...
                    }, m(".pl-s.pr-s.center.small.flex.flex-column.justify-center", {
                        style: {
                ...
                m(".flex.flex-grow", thisPeriod.days.map((weekday, i) => {
    ...
    renderHeaderMobile(thisPageEvents, groupColors, onEventClicked, onEventKeyDown, temporaryEvents) {
        ...
        return m(".calendar-long-events-header.flex-fixed.calendar-hour-margin.pr-l.rel", {
            style: {
    ...
    renderHeaderDesktop(attrs, thisPageEvents, mainPageEvents) {
        ...
        return m(".calendar-long-events-header.flex-fixed.content-bg.pt-s.scrollbar-gutter-stable-or-fallback", [
            ...
            m(".content-bg", [
                m(".calendar-hour-margin.content-bg", {
                    onmousemove: (mouseEvent) => {
    ...
    renderLongEventsForMultipleDays(dayRange, events, groupColors, onEventClicked, onEventKeyDown, temporaryEvents) {
        ...
        const children = layOutEvents(events, zone, (columns) => {
            ...
            return columns.map((rows, c) => rows.map((event) => {
                ...
                }, this.renderLongEventBubble(event, isAllDay ? null : "startAndEndTime" /* EventTextTimeOption.START_END_TIME */, startsBefore, endsAfter, groupColors, onEventClicked, onEventKeyDown, temporaryEvents.includes(event)));
            }));
        }, 1 /* EventLayoutMode.DayBasedColumn */);
        return {
            children,
    ...
    renderDayNamesRow(days, weekIndicator, onDateSelected) {
        ...
        return m(".flex.mb-s", weekIndicator ? m(".calendar-hour-column.calendar-day-indicator.b.center-horizontally", weekIndicator) : m(".calendar-hour-margin"), days.length === 1
            ? null
            : days.map((day) => {
                ...
                return m(".flex.center-horizontally.flex-grow.center.b", [
                    ...
                    m(".rel.click.flex.items-center.justify-center.rel.ml-hpad_small", {
...
//# sourceMappingURL=MultiDayCalendarView.js.map