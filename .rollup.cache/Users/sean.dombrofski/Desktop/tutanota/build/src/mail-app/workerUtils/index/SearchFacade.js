...
import { arrayHash, asyncFind, contains, downcast, getDayShifted, getStartOfDay, isEmpty, isNotNull, isSameTypeRef, neverNull, ofClass, promiseMap, promiseMapCompat, tokenize, TypeRef, uint8ArrayToBase64, } from "@tutao/tutanota-utils";
import { decryptMetaData, decryptSearchIndexEntry, encryptIndexKeyBase64, getIdFromEncSearchIndexEntry, getPerformanceTimestamp, markEnd, markStart, printMeasure, typeRefToTypeInfo, } from "../../../common/api/worker/search/IndexUtils.js";
...
import { INITIAL_MAIL_INDEX_INTERVAL_DAYS } from "./MailIndexer.js";
...
export class SearchFacade {
    ...
    _containsSuggestionToken(entity, model, attributeIds, suggestionToken, matchWordOrder) {
        ...
        if (!attributeIds) {
            ...
            attributeNames = attributeIds.map((id) => neverNull(Object.keys(model.values).find((valueName) => model.values[valueName].id === id) ||
    ...
    _startOrContinueSearch(searchResult, maxResults) {
        ...
        if (searchResult.moreResults.length === 0 && nextScheduledIndexingRun.getTime() > theDayAfterTomorrow.getTime() && !this._mailIndexer.isIndexing) {
            this._mailIndexer.extendIndexIfNeeded(this.userFacade.getLoggedInUser(), getStartOfDay(getDayShifted(new Date(), -INITIAL_MAIL_INDEX_INTERVAL_DAYS)).getTime());
        }
    ...
    _filterByListIdAndGroupSearchResults(indexEntries, searchResult, maxResults) {
        ...
        promiseMap(indexEntries.slice(0, maxResults || indexEntries.length + 1), async (entry, index) => {
...
//# sourceMappingURL=SearchFacade.js.map