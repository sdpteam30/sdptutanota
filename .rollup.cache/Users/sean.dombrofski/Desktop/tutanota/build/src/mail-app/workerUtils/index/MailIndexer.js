import { FULL_INDEXED_TIMESTAMP, MailSetKind, NOTHING_INDEXED_TIMESTAMP, } from "../../../common/api/common/TutanotaConstants";
import { ImportedMailTypeRef, ImportMailStateTypeRef, MailboxGroupRootTypeRef, MailBoxTypeRef, MailDetailsBlobTypeRef, MailDetailsDraftTypeRef, MailFolderTypeRef, MailSetEntryTypeRef, MailTypeRef, } from "../../../common/api/entities/tutanota/TypeRefs.js";
...
import { deconstructMailSetEntryId, elementIdPart, isSameId, LEGACY_BCC_RECIPIENTS_ID, LEGACY_BODY_ID, LEGACY_CC_RECIPIENTS_ID, LEGACY_TO_RECIPIENTS_ID, listIdPart, } from "../../../common/api/common/utils/EntityUtils.js";
import { _createNewIndexUpdate, encryptIndexKeyBase64, filterMailMemberships, getPerformanceTimestamp, htmlToText, typeRefToTypeInfo, } from "../../../common/api/worker/search/IndexUtils.js";
...
import { containsEventOfType } from "../../../common/api/common/utils/EntityUpdateUtils.js";
...
export class MailIndexer {
    ...
    createMailIndexEntries(mail, mailDetails, files) {
        ...
        if (hasSender)
            senderToIndex = getDisplayedSender(mail);
    ...
    async indexMailboxes(user, oldestTimestamp) {
        ...
        try {
            ...
            const error = success ? null : e instanceof ConnectionError ? 1 /* IndexingErrorReason.ConnectionLost */ : 0 /* IndexingErrorReason.Unknown */;
            await this.infoMessageHandler.onSearchIndexStateUpdate({
                initializing: false,
                mailIndexEnabled: this.mailIndexingEnabled,
                progress: 0,
    ...
    async _prepareMailDataForTimeBatch(mboxDataList, timeRange, indexUpdate, indexLoader) {
        ...
        return promiseMap(mboxDataList, (mboxData) => {
            return promiseMap(mboxData.mailListIds.slice(), async (listId) => {
    ...
    async _processIndexMails(mails, indexUpdate, indexLoader) {
        if (this._indexingCancelled)
            throw new CancelledError("cancelled indexing in processing index mails");
    ...
    async processImportStateEntityEvents(events, groupId, batchId, indexUpdate) {
        ...
        await promiseMap(events, async (event) => {
            ...
            if (event.operation === "0" /* OperationType.CREATE */ || event.operation === "1" /* OperationType.UPDATE */) {
                ...
                return await promiseMap(mailIds, (mailId) => this.processNewMail(mailId).then((result) => {
                    if (result) {
                        this._core.encryptSearchIndexEntries(result.mail._id, neverNull(result.mail._ownerGroup), result.keyToIndexEntries, indexUpdate);
                    }
                }));
    ...
    async loadImportedMailIdsInIndexDateRange(importStateId) {
        ...
        if (status !== 3 /* ImportStatus.Finished */ && status !== 2 /* ImportStatus.Canceled */) {
            return Promise.resolve([]);
        }
    ...
    processEntityEvents(events, groupId, batchId, indexUpdate) {
        ...
        return promiseMap(events, (event) => {
            ...
            if (event.operation === "0" /* OperationType.CREATE */) {
                if (containsEventOfType(events, "2" /* OperationType.DELETE */, event.instanceId)) {
                ...
                if (!containsEventOfType(events, "0" /* OperationType.CREATE */, event.instanceId)) {
...
export function _getCurrentIndexTimestamp(groupIndexTimestamps) {
...
//# sourceMappingURL=MailIndexer.js.map