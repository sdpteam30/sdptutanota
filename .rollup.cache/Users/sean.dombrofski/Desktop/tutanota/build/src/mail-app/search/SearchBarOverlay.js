...
export class SearchBarOverlay {
    ...
    _renderProgress(state, attrs) {
        return m(".flex.col.rel", [
            m(".plr-l.pt-s.pb-s.flex.items-center.flex-space-between.mr-negative-s", {
                ...
                m(".top.flex-space-between.col", m(".bottom.flex-space-between", m("", lang.get("indexedMails_label", {
    ...
    renderError(failedIndexingUpTo, attrs) {
        const errorMessageKey = attrs.state.indexState.error === 1 /* IndexingErrorReason.ConnectionLost */ ? "indexingFailedConnection_error" : "indexing_error";
        return m(".flex.rel", [
            m(".plr-l.pt-s.pb-s.flex.items-center.flex-space-between.mr-negative-s", {
                style: {
                    height: px(52),
    ...
    renderContactResult(contact) {
        return [
            ...
            m(".bottom.flex-space-between", m("small.mail-address", contact.mailAddresses && contact.mailAddresses.length > 0 ? contact.mailAddresses[0].address : "")),
        ];
    }
...
//# sourceMappingURL=SearchBarOverlay.js.map