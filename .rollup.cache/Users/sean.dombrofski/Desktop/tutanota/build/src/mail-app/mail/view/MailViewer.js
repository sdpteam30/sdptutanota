...
import { FeatureType, Keys, MailSetKind, SpamRuleType } from "../../../common/api/common/TutanotaConstants";
import { lang } from "../../../common/misc/LanguageViewModel";
import { assertMainOrNode } from "../../../common/api/common/Env";
import { assertNonNull, assertNotNull, defer, noOp, ofClass } from "@tutao/tutanota-utils";
...
export class MailViewer {
    ...
    resizeListener;
    resizeObserverViewport = null; // needed to detect orientation change to recreate pinchzoom at the right time
    resizeObserverZoomable = null; // needed to recreate pinchzoom e.g. when loading images
    viewModel;
    pinchZoomable = null;
    shortcuts;
    scrollDom = null;
    ...
    view(vnode) {
        ...
        return [
            m(".mail-viewer.overflow-x-hidden", [
                ...
                m(".flex-grow.scroll-x.pt.pb.border-radius-big" + (this.viewModel.isContrastFixNeeded() ? ".bg-white.content-black" : " "), {
    ...
    renderMailSubject(attrs) {
        return m("h4.font-weight-600.mt.mb.text-break.selectable." + responsiveCardHMargin(), {
    ...
    renderQuoteExpanderButton() {
        ...
        return m(".abs.flex.justify-center.full-width", {
            style: {
                // +1 for the border
        ...
        ".flex", {
            ...
            onToggled: () => {
                ...
                if (this.shadowDomRoot)
                    this.updateCollapsedQuotes(this.shadowDomRoot, this.shouldDisplayCollapsedQuotes());
            },
            style: {
                height: "24px",
        ...
        return m("#mail-body", {
                ...
                if (client.isMobileDevice()) {
                    ...
                    this.resizeObserverViewport = new ResizeObserver((entries) => {
                        if (this.pinchZoomable) {
                            ...
                            this.createPinchZoom(this.pinchZoomable.getZoomable(), vnode.dom);
                        }
                    });
            ...
            onupdate: (vnode) => {
                ...
                if (!this.bodyLineHeight) {
                    this.updateLineHeight(vnode.dom);
                }
                ...
                if (client.isMobileDevice() && !this.pinchZoomable && this.shadowDomMailContent) {
                    this.createPinchZoom(this.shadowDomMailContent, vnode.dom);
                }
            },
            onbeforeremove: () => {
    ...
    createCollapsedBlockQuote(quote, expanded) {
        ...
        m.render(quoteIndicator, m(Icon, {
            icon: "More" /* Icons.More */,
            class: "icon-xl mlr",
            container: "div",
            style: {
    ...
    eventTargetWithKeyboardInput(target) {
        if (target && target instanceof HTMLElement) {
            return target.matches('input[type="text"], input[type="date"], input[type="datetime-local"], input[type="email"], input[type="month"], input[type="number"],' +
...
//# sourceMappingURL=MailViewer.js.map