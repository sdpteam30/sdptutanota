...
import { assertNotNull, getFirstOrThrow, isSameTypeRef, last, LazyLoaded, lazyMemoized, memoized, noOp, ofClass } from "@tutao/tutanota-utils";
...
import { archiveMails, getConversationTitle, getMoveMailBounds, moveToInbox, showDeleteConfirmationDialog, showMoveMailsDropdown, } from "../../mail/view/MailGuiUtils.js";
...
import { PaidFunctionResult } from "./SearchViewModel.js";
...
import { EventDetailsView, handleEventDeleteButtonClick, handleEventEditButtonClick, handleSendUpdatesClick, } from "../../../calendar-app/calendar/view/EventDetailsView.js";
...
assertMainOrNode();
export class SearchView extends BaseTopLevelView {
    resultListColumn;
    resultDetailsColumn;
    folderColumn;
    viewSlider;
                ...
                return m(FolderColumnView, {
                    ...
                    content: [
                        m(SidebarSection, {
                            ...
                            m(".folder-row.flex-start.mlr-button", m(NavButton, {
                                label: "emails_label",
                                icon: () => "Mail" /* BootIcons.Mail */,
                                // Generate the current url instead of using '#' to avoid Electron reloading the page on click
                                href: this.searchViewModel.getUrlFromSearchCategory("mail" /* SearchCategoryTypes.mail */),
                                click: () => {
                            ...
                            m(".folder-row.flex-start.mlr-button", m(NavButton, {
                                ...
                                href: this.searchViewModel.getUrlFromSearchCategory("contact" /* SearchCategoryTypes.contact */),
                                click: () => {
                                    this.viewSlider.focus(this.resultListColumn);
                                },
                            ...
                            m(".folder-row.flex-start.mlr-button", m(NavButton, {
                                ...
                                href: this.searchViewModel.getUrlFromSearchCategory("calendar" /* SearchCategoryTypes.calendar */),
                                click: () => {
                                    this.viewSlider.focus(this.resultListColumn);
                                },
        ...
        this.resultListColumn = new ViewColumn({
            view: () => {
                return m(BackgroundColumnLayout, {
                    ...
                    desktopToolbar: () => m(DesktopListToolbar, [
                        this.searchViewModel.listModel && getCurrentSearchMode() !== "calendar" /* SearchCategoryTypes.calendar */
                            ? [
    ...
    renderMobileListActionsHeader(header) {
        ...
        if (!isSameTypeRef(this.searchViewModel.searchedType, CalendarEventTypeRef)) {
            rightActions.push(m(EnterMultiselectIconButton, {
                clickAction: () => {
        ...
        return m(BaseMobileHeader, {
            ...
            right: rightActions,
            center: m(".flex-grow.flex.justify-center", {
    ...
    renderMultiSelectMobileHeader() {
        return m(MultiselectMobileHeader, {
            ...
            message: getCurrentSearchMode() === "mail" /* SearchCategoryTypes.mail */
    ...
    renderDetailsView(header) {
        ...
        if (getCurrentSearchMode() === "contact" /* SearchCategoryTypes.contact */) {
            ...
            return m(BackgroundColumnLayout, {
                ...
                m(".fill-absolute.flex.col.overflow-y-scroll", isMultiselect
        ...
        else if (getCurrentSearchMode() === "mail" /* SearchCategoryTypes.mail */) {
            ...
            if (this.searchViewModel.listModel?.state.inMultiselect || !conversationViewModel) {
                ...
                return m(BackgroundColumnLayout, {
                    ...
                    columnLayout: m(MultiItemViewer, {
                        ...
                        loadingAll: this.searchViewModel.loadingAllForSearchResult != null
                            ? "loading"
        ...
        else if (getCurrentSearchMode() === "calendar" /* SearchCategoryTypes.calendar */) {
            ...
            m(ContactViewerActions, { contacts: [], onExport: noOp, onMerge: noOp, onDelete: noOp, onEdit: noOp }), m(".flex-grow.rel.overflow-hidden", m(ColumnEmptyMessageBox, {
    ...
    invalidateBirthdayPreview() {
        if (getCurrentSearchMode() !== "calendar" /* SearchCategoryTypes.calendar */) {
            return;
        }
        const selectedEvent = this.searchViewModel.getSelectedEvents()[0];
    ...
    renderBottomNav() {
        ...
        if (this.viewSlider.focusedColumn === this.resultDetailsColumn && this.searchViewModel.conversationViewModel) {
            ...
            if (getCurrentSearchMode() === "mail" /* SearchCategoryTypes.mail */) {
                ...
                return m(MobileBottomActionBar, m(ContactViewerActions, {
    ...
    shortcuts = lazyMemoized(() => [
        ...
        },
        {
            ...
            help: "archive_action",
            enabled: () => getCurrentSearchMode() === "mail" /* SearchCategoryTypes.mail */,
        },
        {
            key: Keys.I,
                ...
                Dialog.confirm("deleteContacts_msg").then((confirmed) => {
                    ...
                    if (confirmed) {
                        ...
                        for (const contact of selected) {
                            locator.entityClient.erase(contact).catch(ofClass(NotFoundError, (_) => {
...
function getCurrentSearchMode() {
    ...
    if (route.startsWith("/search/contact")) {
        return "contact" /* SearchCategoryTypes.contact */;
    }
    else if (route.startsWith("/search/calendar")) {
        return "calendar" /* SearchCategoryTypes.calendar */;
    }
    else {
        return "mail" /* SearchCategoryTypes.mail */;
    }
}
