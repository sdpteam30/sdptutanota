...
export class EventBanner {
    ...
    ReplyButtons = new LazyLoaded(async () => (await import("../../../calendar-app/calendar/gui/eventpopup/EventPreviewView.js")).ReplyButtons);
    view({ attrs }) {
        const { contents, mail } = attrs;
        if (contents == null || contents.events.length === 0)
            return null;
        const messages = contents.events
            ...
            .filter(isNotNull);
        return messages.map(({ event, message }) => m(InfoBanner, {
            message: () => message,
            ...
            buttons: [
                {
                    ...
                    click: (e, dom) => import("../../../calendar-app/calendar/view/CalendarInvites.js").then(({ showEventDetails }) => showEventDetails(event, dom.getBoundingClientRect(), mail)),
                },
            ],
        }));
    }
...
export function sendResponse(event, recipient, status, previousMail) {
    showProgressDialog("pleaseWait_msg", import("../../../calendar-app/calendar/view/CalendarInvites.js").then(async ({ getLatestEvent }) => {
        ...
        if (replyResult === 1 /* ReplyResult.ReplySent */) {
            ownAttendee.status = status;
        }
        m.redraw();
    }));
}
//# sourceMappingURL=EventBanner.js.map