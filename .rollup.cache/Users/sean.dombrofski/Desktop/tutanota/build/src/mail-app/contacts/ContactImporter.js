...
import { createContact, createContactAddress, createContactCustomDate, createContactMailAddress, createContactMessengerHandle, createContactPhoneNumber, createContactRelationship, createContactWebsite, } from "../../common/api/entities/tutanota/TypeRefs.js";
import m from "mithril";
import { List, ListLoadingState } from "../../common/gui/base/List.js";
...
export class ContactImporter {
    ...
    async importContactsFromFile(vCardData, contactListId) {
        ...
        if (vCardList == null)
            throw new UserError("importVCardError_msg");
    ...
    async importContacts(contacts, contactListId) {
        const importPromise = this.contactFacade
            ...
            .catch(ofClass(ImportError, (e) => Dialog.message(lang.makeTranslation("confirm_msg", lang.get("importContactsError_msg", {
            "{amount}": e.numFailed + "",
            "{total}": contacts.length + "",
        })))))
            .catch(() => Dialog.message("unknownError_msg"));
        await showProgressDialog("pleaseWait_msg", importPromise);
        await Dialog.message(lang.makeTranslation("confirm_msg", lang.get("importVCardSuccess_msg", {
            "{1}": contacts.length,
        })));
    }
    ...
    async importContactsFromDeviceSafely() {
        ...
        const isContactPermissionGranted = await this.systemPermissionHandler.requestPermission("0" /* PermissionType.Contacts */, "grantContactPermissionAction");
    ...
    async importContactsFromDevice() {
        ...
        const allImportableStructuredContacts = (await promiseMap(books, async (book) => await mobileContactsFacade.getContactsInContactBook(book.id, locator.logins.getUserController().loginUsername))).flat();
        const allImportableContacts = new Map(allImportableStructuredContacts.map((structuredContact) => [
 ...
 */
export function showContactImportDialog(contacts, okAction, title) {
    ...
    const dialog = new Dialog("EditSmall" /* DialogType.EditSmall */, {
        view: () => [
            ...
            m(".dialog-max-height.plr-s.pb.text-break.nav-bg", [
                m(".list-bg.border-radius.mt-s.ml-s.mr-s", m(SelectAllCheckbox, {
                    style: {
                        "padding-left": "0",
                    },
...
export async function parseContacts(fileList, fileApp) {
    const rawContacts = [];
...
//# sourceMappingURL=ContactImporter.js.map