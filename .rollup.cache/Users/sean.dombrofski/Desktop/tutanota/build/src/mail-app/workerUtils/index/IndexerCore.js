import { arrayHash, byteLength, defer, findLastIndex, getFromMap, groupByAndMap, lastThrow, mergeMaps, neverNull, noOp, PromisableWrapper, promiseMapCompat, tokenize, TypeRef, uint8ArrayToBase64, } from "@tutao/tutanota-utils";
import { elementIdPart, firstBiggerThanSecond, generatedIdToTimestamp, listIdPart } from "../../../common/api/common/utils/EntityUtils.js";
import { compareMetaEntriesOldest, decryptIndexKey, decryptMetaData, encryptIndexKeyBase64, encryptIndexKeyUint8Array, encryptMetaData, encryptSearchIndexEntry, getIdFromEncSearchIndexEntry, getPerformanceTimestamp, typeRefToTypeInfo, } from "../../../common/api/worker/search/IndexUtils.js";
import { CancelledError } from "../../../common/api/common/error/CancelledError.js";
import { ProgrammingError } from "../../../common/api/common/error/ProgrammingError.js";
import { InvalidDatabaseStateError } from "../../../common/api/common/error/InvalidDatabaseStateError.js";
import { appendBinaryBlocks, calculateNeededSpaceForNumbers, decodeNumbers, encodeNumbers, iterateBinaryBlocks, removeBinaryBlockRanges, } from "../../../common/api/worker/search/SearchIndexEncoding.js";
import { aes256EncryptSearchIndexEntry, unauthenticatedAesDecrypt } from "@tutao/tutanota-crypto";
import { ElementDataOS, GroupDataOS, MetaDataOS, SearchIndexMetaDataOS, SearchIndexOS, SearchIndexWordsIndex, } from "../../../common/api/worker/search/IndexTables.js";
...
export class IndexerCore {
        ...
        return this._executeOperation({
            ...
            operation: (transaction) => {
                ...
                return (this._moveIndexedInstance(indexUpdate, transaction)
                    ...
                    .thenOrApply((rowKeys) => rowKeys && this._insertNewElementData(indexUpdate, transaction, rowKeys))
    ...
    _moveIndexedInstance(indexUpdate, transaction) {
        ...
        if (indexUpdate.move.length === 0)
            return PromisableWrapper.from(undefined); // keep transaction context open (only for Safari)
    ...
    _deleteIndexedInstance(indexUpdate, transaction) {
        ...
        return Promise.all(Array.from(indexUpdate.delete.searchMetaRowToEncInstanceIds).map(([metaRowKey, encInstanceIds]) => this._deleteSearchIndexEntries(transaction, metaRowKey, encInstanceIds)))
        ...
        if (indexOfMetaEntry !== -1) {
            ...
            if (firstEntry) {
                ...
                if (firstEntry.size + toFirstOne.length < SEARCH_INDEX_ROW_LENGTH) {
                    return this._appendIndexEntriesToRow(transaction, metaData, indexOfFirstEntry, toFirstOne).thenOrApply(() => this._writeEntries(transaction, toNextOnes, metaData, appId, typeId));
                }
                else {
    ...
    _appendIndexEntriesToRow(transaction, metaData, metaEntryIndex, entries) {
        ...
        if (metaEntry.size + entries.length > SEARCH_INDEX_ROW_LENGTH) {
            ...
            // sort by id
            // split
            return PromisableWrapper.from(transaction.get(SearchIndexOS, metaEntry.key).then((binaryBlock) => {
                if (!binaryBlock) {
            ...
            return PromisableWrapper.from(transaction.get(SearchIndexOS, metaEntry.key).then((indexEntriesRow) => {
...
//# sourceMappingURL=IndexerCore.js.map