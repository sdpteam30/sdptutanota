...
export function show(props) {
    import("../mail/signature/Signature").then(({ getDefaultSignature }) => {
        ...
        const editor = new HtmlEditor("preview_label")
            ...
            .setEnabled(selectedType === "1" /* EmailSignatureType.EMAIL_SIGNATURE_TYPE_CUSTOM */);
                ...
                return [
                    m(DropDownSelector, {
                        ...
                        selectionChangedHandler: (type) => {
                            if (selectedType === "1" /* EmailSignatureType.EMAIL_SIGNATURE_TYPE_CUSTOM */) {
                                currentCustomSignature = editor.getValue();
                            }
                            selectedType = type;
                            editor.setValue(getSignature(type, defaultSignature, currentCustomSignature));
                            editor.setEnabled(type === "1" /* EmailSignatureType.EMAIL_SIGNATURE_TYPE_CUSTOM */);
                        },
                    }),
                    m(editor),
                ];
        ...
        let editSignatureOkAction = (dialog) => {
            ...
            const updateSignature = () => {
                ...
                if (newType === "1" /* EmailSignatureType.EMAIL_SIGNATURE_TYPE_CUSTOM */) {
            ...
            if (newType === oldType && (newType !== "1" /* EmailSignatureType.EMAIL_SIGNATURE_TYPE_CUSTOM */ || newCustomValue === oldCustomValue)) {
                return dialog.close();
            }
            else {
                if (newType === "1" /* EmailSignatureType.EMAIL_SIGNATURE_TYPE_CUSTOM */ && newCustomValue.length > RECOMMENDED_SIGNATURE_SIZE_LIMIT) {
                    ...
                    const confirmLargeSignatureAttrs = {
                        ...
                        child: {
                            view: () => m("p", lang.get("largeSignature_msg", {
                                "{1}": signatureSizeKb,
                            })),
                        },
                        okAction: (dialog) => {
...
export function getSignatureTypes(props) {
    let signatureTypes = [
        {
            name: lang.get("emailSignatureTypeCustom_msg"),
            value: "1" /* EmailSignatureType.EMAIL_SIGNATURE_TYPE_CUSTOM */,
        },
        {
            name: lang.get("comboBoxSelectionNone_msg"),
            value: "2" /* EmailSignatureType.EMAIL_SIGNATURE_TYPE_NONE */,
        },
    ];
    if (!locator.logins.isEnabled(FeatureType.DisableDefaultSignature) || props.emailSignatureType === "0" /* EmailSignatureType.EMAIL_SIGNATURE_TYPE_DEFAULT */) {
        signatureTypes.splice(0, 0, {
            name: lang.get("emailSignatureTypeDefault_msg"),
            value: "0" /* EmailSignatureType.EMAIL_SIGNATURE_TYPE_DEFAULT */,
        });
    }
    return signatureTypes;
}
function getSignature(type, defaultSignature, currentCustomSignature) {
    if (type === "0" /* EmailSignatureType.EMAIL_SIGNATURE_TYPE_DEFAULT */) {
        return defaultSignature;
    }
    else if (type === "1" /* EmailSignatureType.EMAIL_SIGNATURE_TYPE_CUSTOM */) {
...
export function getSignatureType(props) {
    return neverNull(getSignatureTypes(props).find((t) => t.value === props.emailSignatureType));
}
//# sourceMappingURL=EditSignatureDialog.js.map