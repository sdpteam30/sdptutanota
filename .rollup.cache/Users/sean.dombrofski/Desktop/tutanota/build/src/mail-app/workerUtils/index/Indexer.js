import { ENTITY_EVENT_BATCH_TTL_DAYS, getMembershipGroupType, GroupType, NOTHING_INDEXED_TIMESTAMP, } from "../../../common/api/common/TutanotaConstants.js";
import { ConnectionError, NotAuthorizedError, NotFoundError } from "../../../common/api/common/error/RestError.js";
import { EntityEventBatchTypeRef, UserTypeRef } from "../../../common/api/entities/sys/TypeRefs.js";
import { b64UserIdHash, DbFacade } from "../../../common/api/worker/search/DbFacade.js";
import { assertNotNull, contains, daysToMillis, defer, downcast, getFromMap, isNotNull, isSameTypeRef, isSameTypeRefByAttr, millisToDays, neverNull, noOp, ofClass, promiseMap, } from "@tutao/tutanota-utils";
import { firstBiggerThanSecond, GENERATED_MAX_ID, generatedIdToTimestamp, getElementId, isSameId, timestampToGeneratedId, } from "../../../common/api/common/utils/EntityUtils.js";
...
import { ContactListTypeRef, ContactTypeRef, ImportMailStateTypeRef, MailTypeRef } from "../../../common/api/entities/tutanota/TypeRefs.js";
...
import { aes256EncryptSearchIndexEntry, aes256RandomKey, decryptKey, IV_BYTE_LENGTH, random, unauthenticatedAesDecrypt, } from "@tutao/tutanota-crypto";
import { ElementDataOS, GroupDataOS, Metadata, MetaDataOS, SearchIndexMetaDataOS, SearchIndexOS, SearchIndexWordsIndex, SearchTermSuggestionsOS, } from "../../../common/api/worker/search/IndexTables.js";
import { getIndexerMetaData, updateEncryptionMetadata } from "../../../common/api/worker/facades/lazy/ConfigurationDatabase.js";
import { encryptKeyWithVersionedKey } from "../../../common/api/worker/crypto/CryptoWrapper.js";
const DB_VERSION = 3;
export function newSearchIndexDB() {
...
export class Indexer {
        ...
        try {
            ...
            if (retryOnError !== false && (e instanceof MembershipRemovedError || e instanceof InvalidDatabaseStateError)) {
                ...
                await this.infoMessageHandler.onSearchIndexStateUpdate({
                    ...
                    error: e instanceof ConnectionError ? 1 /* IndexingErrorReason.ConnectionLost */ : 0 /* IndexingErrorReason.Unknown */,
                });
    ...
    async _updateIndexedGroups() {
        ...
        const indexedGroupIds = await promiseMap(await t.getAll(GroupDataOS), (groupDataEntry) => downcast(groupDataEntry.key));
    ...
    _updateGroups(user, groupDiff) {
        if (groupDiff.deletedGroups.some((g) => g.type === GroupType.Mail || g.type === GroupType.Contact)) {
            ...
            return this._loadGroupData(user, groupDiff.newGroups.map((g) => g.id)).then((groupBatches) => {
    ...
    _loadGroupData(user, restrictToTheseGroups) {
        ...
        return promiseMap(memberships, (membership) => {
    ...
    _processEntityEvents(batch) {
        ...
        return this.db.initialized
            ...
            if (filterIndexMemberships(this._initParams.user)
                .map((m) => m.group)
                .indexOf(groupId) === -1) {
                return Promise.resolve();
            }
            ...
            const groupedEvents = new Map(); // define map first because Webstorm has problems with type annotations
    ...
    async _processUserEntityEvents(events) {
        for (const event of events) {
            if (!(event.operation === "1" /* OperationType.UPDATE */ && isSameId(this._initParams.user._id, event.instanceId))) {
                continue;
            }
