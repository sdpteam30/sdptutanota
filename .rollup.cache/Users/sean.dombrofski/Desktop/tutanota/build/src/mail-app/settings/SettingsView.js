...
assertMainOrNode();
export class SettingsView extends BaseTopLevelView {
    viewSlider;
    ...
    constructor(vnode) {
        ...
        this._userFolders = [
            new SettingsFolder(() => "login_label", () => "Contacts" /* BootIcons.Contacts */, "login", () => new LoginSettingsViewer(locator.credentialsProvider, isApp() ? locator.systemFacade : null), undefined),
            new SettingsFolder(() => "email_label", () => "Mail" /* BootIcons.Mail */, "mail", () => new MailSettingsViewer(), undefined),
            new SettingsFolder(() => "contacts_label", () => "Contacts" /* BootIcons.Contacts */, "contacts", () => new ContactsSettingsViewer(), undefined),
        ...
        if (isDesktop()) {
            this._userFolders.push(new SettingsFolder(() => "desktop_label", () => "Desktop" /* Icons.Desktop */, "desktop", () => {
        ...
        mailLocator.mailboxModel.getMailboxDetails().then((mailboxes) => {
            this._userFolders.push(new SettingsFolder(() => "mailImportSettings_label", () => "Import" /* Icons.Import */, "mailImport", () => {
                if (isDesktop()) {
        ...
        this._userFolders.push(new SettingsFolder(() => "mailExportSettings_label", () => "Export" /* Icons.Export */, "mailExport", () => new MailExportViewer(), undefined));
        this._adminFolders = [];
        ...
        this._dummyTemplateFolder = new SettingsFolder(() => "templateGroupDefaultName_label", () => "ListAlt" /* Icons.ListAlt */, {
            folder: "templates",
            id: "init",
        }, () => {
            return {
        ...
        this._settingsColumn = new ViewColumn({
            ...
            view: () => m(BackgroundColumnLayout, {
                ...
                columnLayout: m(".mlr-safe-inset.fill-absolute.content-bg", {
    ...
    async populateAdminFolders() {
        ...
        if (await this.logins.getUserController().canHaveUsers()) {
            this._adminFolders.push(new SettingsFolder(() => "adminUserList_action", () => "Contacts" /* BootIcons.Contacts */, "users", () => new UserListView((viewer) => this.replaceDetailsViewer(viewer), () => this.focusSettingsDetailsColumn(), () => !isApp() && this._customDomains.isLoaded() && this._customDomains.getLoaded().length > 0, () => showUserImportDialog(this._customDomains.getLoaded()), () => exportUserCsv(locator.entityClient, this.logins, locator.fileController, locator.counterFacade)), undefined));
            if (!this.logins.isEnabled(FeatureType.WhitelabelChild)) {
                this._adminFolders.push(new SettingsFolder(() => "sharedMailboxes_label", () => "People" /* Icons.People */, "groups", () => new GroupListView((viewer) => this.replaceDetailsViewer(viewer), () => this.focusSettingsDetailsColumn()), undefined));
            }
        }
        if (this.logins.getUserController().isGlobalAdmin()) {
            this._adminFolders.push(new SettingsFolder(() => "globalSettings_label", () => "Settings" /* BootIcons.Settings */, "global", () => new GlobalSettingsViewer(), undefined));
            if (!this.logins.isEnabled(FeatureType.WhitelabelChild) && !isIOSApp()) {
                this._adminFolders.push(new SettingsFolder(() => "whitelabel_label", () => "Wand" /* Icons.Wand */, "whitelabel", () => new WhitelabelSettingsViewer(locator.entityClient, this.logins), undefined));
            }
        }
        if (!this.logins.isEnabled(FeatureType.WhitelabelChild)) {
            if (this.logins.getUserController().isGlobalAdmin()) {
                this._adminFolders.push(new SettingsFolder(() => "adminSubscription_action", () => "Premium" /* BootIcons.Premium */, "subscription", () => new SubscriptionViewer(currentPlanType, isIOSApp() ? locator.mobilePaymentsFacade : null), undefined));
                this._adminFolders.push(new SettingsFolder(() => "adminPayment_action", () => "CreditCard" /* Icons.CreditCard */, "invoice", () => new PaymentViewer(), undefined));
                this._adminFolders.push(new SettingsFolder(() => "referralSettings_label", () => "Share" /* BootIcons.Share */, "referral", () => new ReferralSettingsViewer(), undefined).setIsVisibleHandler(() => !this.showBusinessSettings()));
                this._adminFolders.push(new SettingsFolder(() => "affiliateSettings_label", () => "Share" /* BootIcons.Share */, "affiliate", () => new AffiliateSettingsViewer(() => this.viewSlider.focusedColumn === this._settingsDetailsColumn, () => {
    ...
    _leaveTemplateGroup(templateInfo) {
        return getConfirmation(lang.getTranslation("confirmLeaveSharedGroup_msg", {
    ...
    _deleteTemplateGroup(templateInfo) {
        return getConfirmation("confirmDeleteTemplateGroup_msg").confirmed(() => showProgressDialog("pleaseWait_msg", locator.serviceExecutor.delete(TemplateGroupService, createUserAreaGroupDeleteData({
    ...
    _aboutThisSoftwareLink() {
        ...
        return m("button.text-center.small.no-text-decoration", {
            style: {
    ...
    async _makeTemplateFolders() {
        ...
        return promiseMap(await loadTemplateGroupInstances(templateMemberships, locator.entityClient), (groupInstance) => {
            ...
            return new SettingsFolder(() => (sharedGroupName ? lang.makeTranslation("templateGroupDefaultName_label", sharedGroupName) : "templateGroupDefaultName_label"), () => "ListAlt" /* Icons.ListAlt */, {
                folder: "templates",
    ...
    async _makeKnowledgeBaseFolders() {
        ...
        if (isCustomizationEnabledForCustomer(customer, FeatureType.KnowledgeBase)) {
            ...
            return promiseMap(await loadTemplateGroupInstances(templateMemberships, locator.entityClient), (groupInstance) => {
                const sharedGroupName = getNullableSharedGroupName(groupInstance.groupInfo, userController.userSettingsGroupRoot, true);
                return new SettingsFolder(() => (sharedGroupName ? lang.makeTranslation("templateGroupDefaultName_label", sharedGroupName) : "templateGroupDefaultName_label"), () => "Book" /* Icons.Book */, {
