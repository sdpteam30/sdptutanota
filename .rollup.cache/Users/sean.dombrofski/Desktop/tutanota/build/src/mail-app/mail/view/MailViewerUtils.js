import { Keys, SYSTEM_GROUP_MAIL_ADDRESS } from "../../../common/api/common/TutanotaConstants";
...
export async function showHeaderDialog(headersPromise) {
    ...
    mailHeadersDialog = Dialog.largeDialog({
        ...
        view: () => m(".white-space-pre.pt.pb.selectable", state.state === "loading" ? m(".center", progressIcon()) : state.headers ?? m(".center", lang.get("noEntries_msg"))),
    })
        .addShortcut({
        key: Keys.ESC,
...
export async function loadMailDetails(mailFacade, mail) {
    if (isDraft(mail)) {
...
export async function editDraft(viewModel) {
...
export async function showSourceDialog(rawHtml) {
    return Dialog.viewerDialog("emailSourceCode_title", SourceCodeViewer, { rawHtml });
}
export function mailViewerMoreActions(viewModel, showReadButton = true) {
    const moreButtons = [];
    if (showReadButton) {
    ...
    if (viewModel.canPersistBlockingStatus() && viewModel.isShowingExternalContent()) {
        moreButtons.push({
            ...
            click: () => viewModel.setContentBlockingStatus("0" /* ContentBlockingStatus.Block */),
    ...
    if (viewModel.canPersistBlockingStatus() && viewModel.isBlockingExternalImages()) {
        moreButtons.push({
            ...
            click: () => viewModel.setContentBlockingStatus("1" /* ContentBlockingStatus.Show */),
            icon: "Picture" /* Icons.Picture */,
        });
    }
...
function reportMail(viewModel) {
    ...
    const dialog = Dialog.showActionDialog({
        ...
        child: () => m(".flex.col.mt-m", {
            ...
            ifAllowedTutaLinks(locator.logins, "https://tuta.com/faq#phishing" /* InfoLink.Phishing */, (link) => m(ExternalLink, {
                href: link,
...
export function getRecipientHeading(mail, preferNameOnly) {
...
export function getSenderOrRecipientHeading(mail, preferNameOnly) {
    if (isSystemNotification(mail)) {
        return "";
    }
    else if (mail.state === "2" /* MailState.RECEIVED */) {
...
export var MailFilterType;
(function (MailFilterType) {
    MailFilterType[MailFilterType["Unread"] = 0] = "Unread";
    MailFilterType[MailFilterType["Read"] = 1] = "Read";
    MailFilterType[MailFilterType["WithAttachments"] = 2] = "WithAttachments";
})(MailFilterType || (MailFilterType = {}));
export function getMailFilterForType(filter) {
    switch (filter) {
 ...
 */
export function isRepliedTo(mail) {
    return mail.replyType === "1" /* ReplyType.REPLY */ || mail.replyType === "3" /* ReplyType.REPLY_FORWARD */;
}
export function canDoDragAndDropExport() {
    return isDesktop();
}
//# sourceMappingURL=MailViewerUtils.js.map