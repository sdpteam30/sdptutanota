...
assertMainOrNode();
export class ContactView extends BaseTopLevelView {
    listColumn;
    folderColumn;
    viewSlider;
    contactViewModel;
    ...
    renderContactListColumn(header) {
        return m(BackgroundColumnLayout, {
            ...
            mobileHeader: () => this.contactViewModel.listModel.state.inMultiselect
    ...
    renderContactListRecipientColumn(header) {
        return m(BackgroundColumnLayout, {
            ...
            desktopToolbar: () => this.renderListToolbar(),
            mobileHeader: () => this.contactListViewModel.listModel?.state.inMultiselect
                ? m(MultiselectMobileHeader, {
    ...
    detailsViewerActions() {
        if (this.inContactListView()) {
            ...
            return m(ContactViewerActions, {
                ...
                onDelete: (contacts) => deleteContacts(contacts, () => this.contactViewModel.listModel.selectNone()),
    ...
    view({ attrs }) {
        ...
        return m("#contact.main-view", m(this.viewSlider, {
            header: styles.isSingleColumnLayout()
                ? null
                : m(Header, {
                    searchBar: () => this.inContactListView()
                        ? null
                        : m(LazySearchBar, {
            ...
            bottomNav: styles.isSingleColumnLayout() && this.viewSlider.focusedColumn === this.detailsColumn && !this.showingListView()
        ...
        if (this.inContactListView()) {
            ...
            return this.contactListViewModel.listModel == null || this.showingListView()
                ? m(ColumnEmptyMessageBox, {
                    ...
                    bottomContent: entries.length > 0
                        ? m(Button, {
                            label: "cancel_action",
    ...
    getShortcuts() {
        let shortcuts = [
            ...listSelectionKeyboardShortcuts(1 /* MultiselectMode.Enabled */, () => {
    ...
    getContactListInvitationRows() {
        import("../../../common/sharing/view/GroupInvitationFolderRow.js")
            ...
            this.invitationRows = this.contactListViewModel.getContactListInvitations().map((invitation) => m(GroupInvitationFolderRow, {
                invitation,
            }));
        })
            .then(m.redraw);
                ...
                return [
                    ...
                    contactListInfo.isOwner
                        ...
                        : {
                            ...
                            icon: "Trash" /* Icons.Trash */,
                            click: async () => {
                                if (await Dialog.confirm(lang.makeTranslation("confirm_msg", lang.get("confirmLeaveSharedGroup_msg", { "{groupName}": contactListInfo.name })))) {
        ...
        return showProgressDialog("pleaseWait_msg", locator.contactModel.getContactListId().then((contactListId) => {
            ...
            if (allContacts.length === 0) {
                ...
                if (mergeableAndDuplicates.deletable.length > 0) {
                    deletePromise = Dialog.confirm(lang.makeTranslation("confirm_msg", lang.get("duplicatesNotification_msg", {
    ...
    _showMergeDialogs(mergable) {
        ...
        if (mergable.length > 0) {
            ...
            return mergeDialog
                ...
                if (action === "merge" /* ContactMergeAction.Merge */) {
                ...
                else if (action === "deleteFirst" /* ContactMergeAction.DeleteFirst */) {
                    this._removeFromMergableContacts(mergable, contact1);
                    return locator.entityClient.erase(contact1);
                }
                else if (action === "deleteSecond" /* ContactMergeAction.DeleteSecond */) {
                    this._removeFromMergableContacts(mergable, contact2);
                    return locator.entityClient.erase(contact2);
                }
                else if (action === "skip" /* ContactMergeAction.Skip */) {
                    this._removeFromMergableContacts(mergable, contact2);
                }
                else if (action === "cancel" /* ContactMergeAction.Cancel */) {
                    clear(mergable);
                    canceled = true;
                }
            })
    ...
    renderListToolbar() {
        if (this.inContactListView()) {
            ...
            return m(DesktopListToolbar, m(SelectAllCheckbox, selectionAttrsForList(this.contactListViewModel.listModel)), m(".flex-grow"), this.canEditSelectedContactList()
                ? m(IconButton, {
                    title: "addEntries_action",
                    icon: "Add" /* Icons.Add */,
                    click: () => {
    ...
    renderSortByButton() {
        return m(IconButton, {
            title: "sortBy_label",
            icon: "ListOrdered" /* Icons.ListOrdered */,
            click: (e, dom) => {
                createDropdown({
                    lazyButtons: () => [
                        {
...
export function writeMail(to, subject = "") {
    return locator.mailboxModel.getUserMailboxDetails().then((mailboxDetails) => {
        return newMailEditorFromTemplate(mailboxDetails, {
            to: [to],
        }, subject, appendEmailSignature("", locator.logins.getUserController().props)).then((editor) => editor.show());
    });
}
export function deleteContacts(contactList, onConfirm = noOp) {
...
export function confirmMerge(keptContact, goodbyeContact) {
